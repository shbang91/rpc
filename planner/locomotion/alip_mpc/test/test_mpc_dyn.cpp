...
void func_to_test(test_data_t **tdata, input_data_t indata,
                  output_data_t outdata, int i) {
  // x_lip_current
...
void initialize_data(input_data_t &indata, output_data_t &outdata, double Lx,
                     double Ly) {
...
int main() {
  ...
  if (file.is_open()) {
    ...
    file.write(reinterpret_cast<char *>(&xlip_size),
               sizeof(size_t)); // reinterpret_cast is used to convert the
                                // size_t variables and vector data to char* for
                                // binary file writing
    file.write(reinterpret_cast<char *>(&ufp_size), sizeof(size_t));

    // Save vector data
    file.write(reinterpret_cast<char *>(fullmpcdata.xlip_sol.data()),
               xlip_size * sizeof(double));
    file.write(reinterpret_cast<char *>(fullmpcdata.ufp_sol.data()),
               ufp_size * sizeof(double));
  ...
  // cout << "ufp sol:" << endl;
  // for (int i = 0; i < stoi(step_horizon); i++) cout <<
  // fullmpcdata.ufp_sol[2*i] << " " << fullmpcdata.ufp_sol[2*i+1] << endl; cout
  // << endl; cout << "x + ufp" << endl;
  /*
  for (int i = 0; i < stoi(step_horizon); i++){
      cout << fullmpcdata.xlip_sol[4*i]+fullmpcdata.ufp_sol[2*i];
      cout << "  " << fullmpcdata.xlip_sol[4*i+1]+fullmpcdata.ufp_sol[2*i+1] <<
  endl;
  }
  cout << " L: " << endl;
  for (int i = 0; i < stoi(step_horizon) ;i++){
