...
FrictionCone::FrictionCone(const double mu, const double fzmin,
    ...
    : mu_(mu), fzmin_(fzmin), fzmax_(fzmax), x_(x), y_(y),
      wrench_cone_(MatrixXd::Zero(16 * 2, 12)) {
  try {
    if (mu <= 0.0) {
  ...
  for (int i = 0; i < 2; ++i) {
    wrench_cone_.block(16 * i, 6 * i, 16, 6) = cone;
  }
}
...
void FrictionCone::setQP(QPData &qp_data) const {
  ...
  for (int i = 0; i < qp_data.dim_.N; ++i) {
    ...
    if (num_conatcts > 0) {
      qp_data.qp_[i].D = wrench_cone_.topLeftCorner(qp_data.qp_[i].D.rows(),
                                                    qp_data.qp_[i].D.cols());
    }
    qp_data.qp_[i].lg.setZero();
    qp_data.qp_[i].ug = Eigen::VectorXd::Constant(qp_data.dim_.ng[i], soft_inf);
    qp_data.qp_[i].ug_mask =
        Eigen::VectorXd::Constant(qp_data.dim_.ng[i], 0.0); // disable upper
    // bounds
  }
}
