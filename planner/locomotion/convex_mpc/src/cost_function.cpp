...
CostFunction::CostFunction(const double dt, const Matrix6d &Qqq,
    ...
    : dt_(dt), Qqq_(Qqq), Qvv_(Qvv), Quu_(Matrix12d::Zero()),
      base_pose_(Vector7d::Zero()), base_pose_ref_(), single_rigid_body_(),
      qdiff_(Vector6d::Zero()), Jqdiff_(Matrix6d::Zero()),
...
void CostFunction::initQP(QPData &qp_data) {
  for (int i = 0; i < qp_data.dim_.N + 1; ++i) {
    qp_data.qp_[i].Q.template topLeftCorner<6, 6>() = Qqq_;
    qp_data.qp_[i].Q.template bottomRightCorner<6, 6>() = Qvv_;
  }
  base_pose_ref_ =
...
void CostFunction::setQP(const ContactSchedule &contact_schedule,
  ...
  v_command_.template tail<3>() = gait_command.vcom;
  v_command_.template head<3>() << 0., 0., gait_command.yaw_rate;
  dq_command_ = dt_ * v_command_;
  base_pose_.template tail<3>() = robot_state.com();
  base_pose_.template head<4>() = robot_state.quat().coeffs();
  ...
  base_pose_ref_[0].template tail<3>() = robot_state.com();
  base_pose_ref_[0].template head<4>() = Quaterniond(R_ref).coeffs();
  ...
  for (int i = 0; i < qp_data.dim_.N + 1; ++i) {
    ...
    std::cout << "robot state twist: " << robot_state.twist().transpose()
              << std::endl;
  }
  for (int i = 0; i < qp_data.dim_.N; ++i) {
