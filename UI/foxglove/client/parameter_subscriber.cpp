...
FoxgloveParameterSubscriber::FoxgloveParameterSubscriber(
  ...
  client_.setBinaryMessageHandler([&](const uint8_t *data, size_t dataLength) {
    if (static_cast<foxglove::BinaryOpcode>(data[0]) !=
        foxglove::BinaryOpcode::MESSAGE_DATA) {
      return;
    }

    client_.getParameters({"n_steps", "t_ss", "t_ds", "torso_ori_task_weight",
                           "torso_ori_task_kp", "torso_ori_task_kd",
                           "foot_pos_task_weight",
                           "foot_pos_task_weight_at_swing", "foot_pos_task_kp",
                           "foot_pos_task_kd", "foot_ori_task_weight",
                           "foot_ori_task_weight_at_swing", "foot_ori_task_kp",
                           "foot_ori_task_kd"});
  });
  ...
  client_.setTextMessageHandler([&](const std::string &payload) {
    ...
    if (op != "advertise") {
      if (op == "parameterValues") {
        auto param_value =
            msg["parameters"].get<std::vector<foxglove::Parameter>>();
        for (const auto &param : param_value) {
          // assign new parameter value according to specified Map Type (after
          // parsing from Foxglove)
          // ------ process as int --------
          if (parameters_map_int.count(param.getName())) {
            ...
            *parameters_map_double[param.getName()] = double(p_val);
            ...
            // loop through left and right feet managers to set new weight
            // values
            _UpdateTaskHierarchyManager(param, "pos", parameters_map_hm);
          } else if (param.getName().find("foot_ori_task") !=
                     std::string::npos) {
            // loop through left and right feet managers to set new weight
            // values
            _UpdateTaskHierarchyManager(param, "ori", parameters_map_hm);
          } else { // ------ process as task --------
            // loop through all tasks to find the one that matches the parameter
            // name
    ...
    std::vector<std::pair<foxglove::SubscriptionId, foxglove::ChannelId>>
        subscribePayload;
...
FoxgloveParameterSubscriber::~FoxgloveParameterSubscriber() { client_.close(); }

void FoxgloveParameterSubscriber::UpdateParameters() {
  // add here parameters that need to be interpolated from current to desired
  // value
}

double FoxgloveParameterSubscriber::_ParseFoxgloveParameter(
    const foxglove::Parameter &param) {
...
Eigen::Vector3d FoxgloveParameterSubscriber::_ParseFoxgloveParameterVec(
    const foxglove::Parameter &param) {
  Eigen::Vector3d vec;
  if (param.getType() == foxglove::ParameterType::PARAMETER_ARRAY) {
    ...
    for (const auto &pp_val : p_val) {
      if (pp_val.getType() == foxglove::ParameterType::PARAMETER_DOUBLE) {
        ...
        vec[i_x] = double(pp_val.getValue<int64_t>());
      i_x++;
    }
  }
...
void FoxgloveParameterSubscriber::_UpdateTaskHierarchyManager(
