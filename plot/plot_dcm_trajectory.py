...
def plot_foot(ax, pos, ori, color, text):
    foot_half_len = 0.21 / 2.0
    foot_half_wid = 0.043
    foot_half_len_ctrl = 0.08
    foot_half_wid_ctrl = 0.035
    if text:
    ...
    d = -pos.dot(normal)
    xx, yy = np.meshgrid(
        np.linspace(-foot_half_len, foot_half_len, 2),
    ...
    xx += pos[0]
    yy += pos[1]
    xx_ctrl, yy_ctrl = np.einsum(
        "ji, mni->jmn", rmat[0:2, 0:2], np.dstack([xx_ctrl, yy_ctrl])
    )
    xx_ctrl += pos[0]
    yy_ctrl += pos[1]
    ...
    if text:
        ...
        ax.scatter(
            xs=pos[0], ys=pos[1], zs=pos[2], zdir="z", s=50, c=color, depthshade=True
        )
    else:
...
def main(args):
    ...
    with open("experiment_data/pnc.pkl", "rb") as file:
        while True:
            try:
                d = pickle.load(file)
                time.append(d["time"])
                icp_act.append(d["est_icp"])
                icp_des.append(d["des_icp"])
                lfoot_pos_act.append(d["lfoot_pos"])
                rfoot_pos_act.append(d["rfoot_pos"])
    ...
    initial_step_idx = np.where(
        np.abs(np.array(time) - initial_time)
        == np.min(np.abs(np.array(time) - initial_time))
    )[0][0]
    ...
    first_step_idx = np.where(
        np.abs(np.array(time) - time_at_touch_down)
        == np.min(np.abs(np.array(time) - time_at_touch_down))
    )[0][0]
    first_liftoff_idx = np.where(
        np.abs(np.array(time) - time_at_swing)
        == np.min(np.abs(np.array(time) - time_at_swing))
    )[0][0]
    ...
    if b_plot_2D:
        ...
        plt.plot(
            icp_act[first_step_idx, 0],
            icp_act[first_step_idx, 1],
            "co",
            label="ICP end",
        )
        plt.plot(
            icp_act[initial_step_idx:first_step_idx, 0],
            icp_act[initial_step_idx:first_step_idx, 1],
            color="c",
        )

        plt.plot(
            liftoff_icp_pos_act[0],
            liftoff_icp_pos_act[1],
            "c*",
            label="ICP liftoff(act)",
        )
        plt.plot(
            liftoff_icp_pos_des[0],
        ...
        plt.plot(
            cmp_des[first_liftoff_idx:first_step_idx, 0],
        ...
        # plot feet position
        plt.plot(
            lfoot_pos_act[initial_step_idx, 0],
        ...
        plt.plot(
            rfoot_pos_act[initial_step_idx, 0],
        ...
        # plot footstep plan
        plt.plot(
            curr_lfoot_contact_pos[0, 0],
            curr_lfoot_contact_pos[0, 1],
            "rs",
            label="initLF",
        )
        plt.plot(
            curr_rfoot_contact_pos[0, 0],
        ...
        plot_foot(
            ax,
            np.squeeze(curr_lfoot_contact_pos),
            np.squeeze(curr_lfoot_contact_ori),
            colors[0],
            False,
        )
        plot_foot(
            ax,
            np.squeeze(curr_rfoot_contact_pos),
            np.squeeze(curr_rfoot_contact_ori),
            colors[1],
            False,
        )
