...
base_estimator_type = {'est', 'gt'};    % 'est' = kinematics-only
...
%%
num_wbc_data = length(com_vel_est);
...
for i = 1:3
    ...
    grid on
    hold on
    plot(wbc_time, base_joint_pos_kf(i, :), 'r', 'LineWidth',2);
    min_val = min([pos_est(i,:), base_joint_pos_kf(i,:)]);
    max_val = max([pos_est(i,:), base_joint_pos_kf(i,:)]);
...
for i = 1:3
    ...
    grid on
    hold on
    plot(wbc_time, base_ori_rpy_from_imu(i, :), 'r', 'LineWidth',2);
    min_val = min([rpy_est(i,:), base_ori_rpy_from_imu(i,:)]);
    max_val = max([rpy_est(i,:), base_ori_rpy_from_imu(i,:)]);
    min_val = min_val - 0.1 * (max_val - min_val);
    max_val = max_val + 0.1 *(max_val - min_val);
    set_fig_opt()
    plot_phase(time, state, min_val, max_val, phase_color)
...
for i = 3:-1:1
    j = j+1;
    subplot(3, 1, j);
    plot(wbc_time, imu_ori_rpy(i, :), 'b', 'LineWidth',2);
    grid on
    hold on
    min_val = min(imu_ori_rpy(i,:));
    max_val = max(imu_ori_rpy(i,:));
    min_val = min_val - 0.1 * (max_val - min_val);
    max_val = max_val + 0.1 *(max_val - min_val);
    set_fig_opt()
    plot_phase(time, state, min_val, max_val, phase_color)
    xlabel('time')
    ylabel(rpy_label(j))
    sgtitle('imu ori rpy (IMU Frame Quat)', 'FontSize', 30)
end
...
for i = 1:3
    ...
    grid on
    hold on
    plot(wbc_time, base_joint_lin_vel_kf(i, :), 'r', 'LineWidth',2);
    min_val = min([lin_vel_est(i,:), base_joint_lin_vel_kf(i,:)]);
    max_val = max([lin_vel_est(i,:), base_joint_lin_vel_kf(i,:)]);
    min_val = min_val - 0.1 * (max_val - min_val);
    max_val = max_val + 0.1 *(max_val - min_val);
    set_fig_opt()
    plot_phase(time, state, min_val, max_val, phase_color)
...
for i = 1:3
    ...
    grid on
    hold on
    plot(wbc_time, imu_ang_vel(i, :), 'r', 'LineWidth',2);
    min_val = min([ang_vel_est(i,:), imu_ang_vel(i,:)]);
    max_val = max([ang_vel_est(i,:), imu_ang_vel(i,:)]);
    min_val = min_val - 0.1 * (max_val - min_val);
    max_val = max_val + 0.1 *(max_val - min_val);
    set_fig_opt()
    plot_phase(time, state, min_val, max_val, phase_color)
...
for i = 1:3
    subplot(3, 1, i);
    plot(wbc_time, imu_ang_vel(i, :), 'b', 'LineWidth',2);
    grid on
    hold on
    min_val = min(imu_ang_vel(i,:));
    max_val = max(imu_ang_vel(i,:));
    min_val = min_val - 0.1 * (max_val - min_val);
    max_val = max_val + 0.1 *(max_val - min_val);
    set_fig_opt()
    plot_phase(time, state, min_val, max_val, phase_color)
    xlabel('time')
    ylabel(ang_vel_label(i))
    sgtitle('imu ang vel', 'FontSize', 30)
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% com vel est
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
