...
//import { toString } from "eslint-plugin-prettier";
...
function ExamplePanel({ context }: { context: PanelExtensionContext }): JSX.Element {
  ...
  useLayoutEffect(() => {
    ...
    //context.subscribe(["steptest"]);

    context.advertise?.("steptest", "steptest");
    context.publish?.("steptest", 69);
  }, [context]);
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", rowGap: "0.2rem" }}>
        ...
        {(topics ?? []).map((topic) => (
          <>
            ...
            <div key={topic.schemaName.length}>{c}</div>
          </>
        ))}
      </div>
