...
const omgidl_parser_1 = require("@foxglove/omgidl-parser");
const generateOmgIdl_1 = require("./generateOmgIdl");
const schemas_1 = require("./schemas");
const testFixtures_1 = require("./testFixtures");
describe("generateOmgIdl", () => {
    it("generates .idl files", () => {
        expect((0, generateOmgIdl_1.generateOmgIdl)(testFixtures_1.exampleEnum)).toMatchInlineSnapshot(`
      ...
      "
    `);
        expect((0, generateOmgIdl_1.generateOmgIdl)(testFixtures_1.exampleMessage)).toMatchInlineSnapshot(`
      "// Generated by https://github.com/foxglove/schemas
    ...
    const allIdlFiles = new Map([
        ["Time", generateOmgIdl_1.TIME_IDL],
        ["Duration", generateOmgIdl_1.DURATION_IDL],
        ...Object.entries(schemas_1.foxgloveMessageSchemas).map(([name, schema]) => [
            name,
            (0, generateOmgIdl_1.generateOmgIdl)(schema),
        ]),
        ...Object.entries(schemas_1.foxgloveEnumSchemas).map(([name, schema]) => [
            name,
            (0, generateOmgIdl_1.generateOmgIdl)(schema),
        ]),
    ]);
    it.each(Object.values(schemas_1.foxgloveMessageSchemas))("generates parseable .idl for $name", (schema) => {
        const includePattern = /^#include "foxglove\/(.*)\.idl"$/gm;
        let idl = (0, generateOmgIdl_1.generateOmgIdl)(schema);
        while (includePattern.test(idl)) {
            idl = idl.replace(includePattern, (_match, name) => {
        ...
        expect(() => (0, omgidl_parser_1.parseIdl)(idl)).not.toThrow();
    });
    it("refuses to generate enum with non-sequential values", () => {
        expect(() => (0, generateOmgIdl_1.generateOmgIdl)({
            type: "enum",
            name: "Foo",
            description: "",
        ...
        })).toThrowErrorMatchingInlineSnapshot(`"Enum value Foo.A at index 0 has value 1; index and value must match for OMG IDL"`);
        expect(() => (0, generateOmgIdl_1.generateOmgIdl)({
            type: "enum",
            name: "Foo",
            description: "",
        ...
        })).toThrowErrorMatchingInlineSnapshot(`"Enum value Foo.B at index 1 has value 3; index and value must match for OMG IDL"`);
    });
});
//# sourceMappingURL=generateOmgIdl.test.js.map