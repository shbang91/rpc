...
exports.generateOmgIdl = exports.DURATION_IDL = exports.TIME_IDL = void 0;
function primitiveToIdl(type) {
    switch (type) {
        case "bytes":
...
`;
function generateOmgIdl(schema) {
    ...
    switch (schema.type) {
        case "enum": {
            const fields = schema.values.map(({ name, value, description }, index) => {
                ...
                if (value !== index) {
                    throw new Error(`Enum value ${schema.name}.${name} at index ${index} has value ${value}; index and value must match for OMG IDL`);
                }
...
exports.generateOmgIdl = generateOmgIdl;
//# sourceMappingURL=generateOmgIdl.js.map