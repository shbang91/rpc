...
#if B_USE_ZMQ
#include "controller/med7_controller/med7_data_manager.hpp"
#endif
...
Med7Controller::Med7Controller(Med7TCIContainer *tci_container,
    ...
    : tci_container_(tci_container), robot_(robot),
      ...
      joint_trq_cmd_prev_(Eigen::VectorXd::Zero(med7::n_adof)),
      wbc_qddot_cmd_(Eigen::VectorXd::Zero(med7::n_qdot)), b_sim_(true),
      b_first_visit_pos_ctrl_(true), b_first_visit_wbc_ctrl_(true),
      b_smoothing_command_(false), smoothing_command_duration_(0.),
      init_joint_pos_(Eigen::VectorXd::Zero(med7::n_adof)) {
  // init_joint_pos_(Eigen::VectorXd::Zero(med7::n_adof)) {
  util::PrettyConstructor(2, "Med7Controller");
  sp_ = Med7StateProvider::GetStateProvider();

#if B_USE_MATLOGGER
  logger_ = XBot::MatLogger2::MakeLogger("/tmp/med7_controller_data");
#endif

  // actuated selection matrix
  // sa_ = Eigen::MatrixXd::Identity(
  // med7::n_adof, med7::n_adof); // all joints are actuated
  sa_ = Eigen::MatrixXd::Identity(med7::n_adof,
                                  med7::n_adof); // all joints are actuated
  ...
  try {
    YAML::Node cfg = YAML::LoadFile(THIS_COM "config/med7/ihwbc_gains.yaml");

    // initialize med7 controller params
...
Med7Controller::~Med7Controller() {
  delete ihwbc_;
  delete joint_integrator_;
}

void Med7Controller::GetCommand(void *command) {
  if (sp_->state_ == med7_states::kStandUp) {
    ...
    if (b_first_visit_wbc_ctrl_) {
      ...
      joint_integrator_->Initialize(init_joint_pos_,
                                    Eigen::VectorXd::Zero(med7::n_adof));
      // erase jpos task
  ...
  static_cast<Med7Command *>(command)->joint_pos_cmd_ = joint_pos_cmd_;
  static_cast<Med7Command *>(command)->joint_vel_cmd_ = joint_vel_cmd_;
  static_cast<Med7Command *>(command)->joint_trq_cmd_ = joint_trq_cmd_;
