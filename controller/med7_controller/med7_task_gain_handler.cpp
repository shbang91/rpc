...
Med7TaskGainHandler::Med7TaskGainHandler(Med7ControlArchitecture *ctrl_arch)
    : ctrl_arch_(ctrl_arch), b_signal_received_(false), b_first_visit_(false),
      b_com_xy_task_(false), count_(0) {

  util::PrettyConstructor(1, "Med7TaskGainHandler");
  std::string border = "=";
...
void Med7TaskGainHandler::Trigger(const std::string &task_name,
...
void Med7TaskGainHandler::Process() {
  count_ += 1;
...
void Med7TaskGainHandler::_ResetParams() {
...
bool Med7TaskGainHandler::_TaskExists(const std::string &task_name) {
  ...
  if (task_map.find(task_name) == task_map.end()) {
    std::cout << "================================================"
              << std::endl;
    std::cout << "Med7TaskGainHandler: Task not found" << std::endl;
    std::cout << "================================================"
              << std::endl;
    return false;
  }
