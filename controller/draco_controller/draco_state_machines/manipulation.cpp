...
void Manipulation::FirstVisit() {
  ...
  //ctrl_arch_->rh_SE3_tm_->UpdateDesired(
  //    target_rh_iso);
  //ctrl_arch_->lh_SE3_tm_->UpdateDesired(
  //    target_lh_iso);
...
void Manipulation::OneStep() {
 // std::cout << "Manipulation OneStep called" << std::endl;
  ...
  if (state_id_ == draco_states::kDoubleSupportBalance) {
    ...
    ctrl_arch_->lh_pos_hm_->UpdateRampToMin(background_start_time_);
    ctrl_arch_->lh_ori_hm_->UpdateRampToMin(background_start_time_);
    ctrl_arch_->rh_pos_hm_->UpdateRampToMin(background_start_time_);
    ctrl_arch_->rh_ori_hm_->UpdateRampToMin(background_start_time_);
  }
