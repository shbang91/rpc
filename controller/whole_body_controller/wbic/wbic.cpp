...
WBIC::WBIC(const std::vector<bool> &act_qdot_list, QPParams *qp_params)
    : WBC(act_qdot_list), threshold_(0.001) {
  util::PrettyConstructor(3, "WBIC");

  wbic_data_ = new WBICData(num_floating_, num_qdot_, qp_params);
}

WBIC::~WBIC() { delete wbic_data_; }

bool WBIC::FindConfiguration(
    const Eigen::VectorXd &curr_jpos, const std::vector<Task *> &task_vector,
    const std::vector<Contact *> &contact_vector,
    const std::vector<InternalConstraint *> &internal_constraint_vector,
    Eigen::VectorXd &jpos_cmd, Eigen::VectorXd &jvel_cmd,
    Eigen::VectorXd &wbc_qddot_cmd) {

  // check task and contact
  assert(task_vector.size() > 0);
  b_contact_ = contact_vector.size() > 0 ? true : false;
  ...
  if (b_internal_constraint_) {
    // build Ji, Jidot_qdot_vec
    int row_idx(0);
    for (const auto ic_ptr : internal_constraint_vector) {
    ...
    // build Ni
    _BuildProjectionMatrix(Ji, Ni);
    // build Ni_dyn_
    _BuildProjectionMatrix(Ji, Ni_dyn_, &Minv_);
  }
  ...
  if (b_contact_) {
    ...
    // contact projection
    Eigen::MatrixXd JcNi = Jc * Ni; // contact jac projected to internal
                                    // constraints
    ...
    Ni_Nci_dyn_ = N_pre_dyn;         // saving for gravity comp (not essential)
    //  constraint
...
void WBIC::SetParameters(const YAML::Node &node) {
  try {
