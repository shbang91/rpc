...
void ForceTrajectoryManager::_ConvertToGlobalDesired(
    Eigen::VectorXd &des_traj_w) {
  Eigen::MatrixXd rot_l_w =
      Eigen::MatrixXd::Zero(des_traj_w.size(), des_traj_w.size());
  rot_l_w.block<3, 3>(0, 0) =
      robot_->GetLinkIsometry(force_task_->contact()->TargetLinkIdx()).linear();
