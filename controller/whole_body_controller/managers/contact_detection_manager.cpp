...
ContactDetectionManager::ContactDetectionManager(PinocchioRobotSystem *_robot)
  ...
  robot_ = _robot;
  wait_count_post_transition_ =
      100; // wait this many control ticks before re-transitioning
  ...
  try {
    ...
    b_debug_only_log_ =
        util::ReadParameter<bool>(cfg["contact_detection"], "b_debug_only_log");
    b_use_contact_sensor_ = util::ReadParameter<bool>(cfg["contact_detection"],
                                                      "b_use_contact_sensor");
    b_use_foot_height_ = util::ReadParameter<bool>(cfg["contact_detection"],
                                                   "b_use_foot_height");
    ...
    auto contact_limits = util::ReadParameter<double>(cfg["wbc"]["contact"],
                                                      prefix + "_max_rf_z");
    ...
    foot_height_tol_ = util::ReadParameter<double>(cfg["contact_detection"],
                                                   prefix + "_foot_height_tol");
    ...
    volt_bias_ = util::ReadParameter<Eigen::Vector2d>(cfg["contact_detection"],
                                                      "volt_bias");
    ...
    auto b_swing_side =
        util::ReadParameter<int>(cfg["dcm_walking"], "first_swing_leg");
    ...
    Eigen::Vector2d l_front_pos =
        Eigen::Vector2d(foot_half_length, foot_half_width);
    Eigen::Vector2d r_front_pos =
        Eigen::Vector2d(foot_half_length, -foot_half_width);
    Eigen::Vector2d r_back_pos =
        Eigen::Vector2d(-foot_half_length, -foot_half_width);
    Eigen::Vector2d l_back_pos =
        Eigen::Vector2d(-foot_half_length, foot_half_width);
...
void ContactDetectionManager::_CheckSwingFootContact(
    const double expected_height_difference) {

  // get swing and support position and orientation
  Eigen::Vector3d swing_foot_pos =
      robot_->GetLinkIsometry(swing_leg_name_).translation();
  Eigen::Matrix3d swing_foot_ori =
      robot_->GetLinkIsometry(swing_leg_name_).linear();
  Eigen::Vector3d support_foot_pos =
      robot_->GetLinkIsometry(support_leg_name_).translation();
  Eigen::Matrix3d support_foot_ori =
      robot_->GetLinkIsometry(support_leg_name_).linear();
  ...
  for (auto const &[num, pos] : foot_corner_map_) {
    ankle_to_corner_height =
        swing_foot_ori(2, 0) * pos(0) + swing_foot_ori(2, 1) * pos(1);
...
void ContactDetectionManager::_ConvertVoltageToForce(
    const Eigen::Vector2d &contact_sensor_v,
    Eigen::Vector2d &contact_force_return) {
  contact_force_return =
      volt_to_force_map_.cwiseProduct(contact_sensor_v) + volt_bias_;
}

void ContactDetectionManager::_CheckContactThresholds(
    const DracoStateProvider *_sp) {
  // Schmitt trigger

  // Left foot (add !_sp->b_lf_contact_ for ONLY early contact)
  if ((contact_forces_filt_(end_effector::LFoot) <
       schmitt_thresholds_(LimitsIdx::low))) {
    // if previously in contact and LOWER force threshold reached, change to not
    // in contact
    b_contact_touchdown_[end_effector::LFoot] = false;
  } else if ((contact_forces_filt_(end_effector::LFoot) >
              schmitt_thresholds_(LimitsIdx::high))) {
    // if previously NOT in contact and UPPER force threshold reached, change to
    // in contact
    b_contact_touchdown_[end_effector::LFoot] = true;
  }

  // Right foot
  if ((contact_forces_filt_(end_effector::RFoot) <
       schmitt_thresholds_(LimitsIdx::low))) {
    // if previously in contact and LOWER force threshold reached, change to not
    // in contact
    b_contact_touchdown_[end_effector::RFoot] = false;
  } else if ((contact_forces_filt_(end_effector::RFoot) >
              schmitt_thresholds_(LimitsIdx::high))) {
    // if previously NOT in contact and UPPER force threshold reached, change to
    // not in contact
...
void ContactDetectionManager::UpdateForceMeasurements(
    const Eigen::Vector2d &contact_normal_raw) {
...
bool ContactDetectionManager::UpdateContactStates(
    DracoStateProvider *_sp, const Eigen::Vector2d &_expected_contact_height) {
  ...
  if (!b_debug_only_log_ &&
      count_post_transition_ > wait_count_post_transition_) {
    ...
    if (swing_leg_name_ == draco_link::l_foot_contact) {
      if (!_sp->b_lf_contact_ && (b_contact_touchdown_[end_effector::LFoot] ||
        ...
        std::cout << "[Contact Detection Manager] Early LF contact"
                  << std::endl;
      }
    } else if (swing_leg_name_ == draco_link::r_foot_contact) {
      if (!_sp->b_rf_contact_ && (b_contact_touchdown_[end_effector::RFoot] ||
        ...
        std::cout << "[Contact Detection Manager] Early RF contact"
                  << std::endl;
      }
    }
  }
