...
void EndEffectorTrajectoryManager::UseCurrent() {
  ...
  Eigen::VectorXd des_pos =
      robot_->GetLinkIsometry(pos_task_->GetTargetIdx()).translation();
  Eigen::VectorXd des_vel =
      robot_->GetLinkSpatialVel(pos_task_->GetTargetIdx()).tail<3>();
  Eigen::Quaterniond des_ori_quat(
      robot_->GetLinkIsometry(ori_task_->GetTargetIdx()).linear());
  Eigen::VectorXd des_ang_vel =
      robot_->GetLinkSpatialVel(ori_task_->GetTargetIdx()).head<3>();

  Eigen::VectorXd des_ori(des_ori_quat.coeffs());

  pos_task_->UpdateDesiredTask(des_pos, des_vel, Eigen::VectorXd::Zero(3));
  ori_task_->UpdateDesiredTask(des_ori, des_ang_vel, Eigen::VectorXd::Zero(3));
}
