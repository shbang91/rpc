...
void ArmTrajectoryManager::UseCurrent() {
...
void ArmTrajectoryManager::UseNominal(const Eigen::Isometry3d &nominal_iso) {
...
void ArmTrajectoryManager::InitializeTrajectory(
...
void ArmTrajectoryManager::UpdateDesired(const double current_time) {
  assert(pos_curve_ != nullptr);
  assert(ori_curve_ != nullptr);

  Eigen::VectorXd des_pos = pos_curve_->Evaluate(current_time);
  Eigen::VectorXd des_vel = pos_curve_->EvaluateFirstDerivative(current_time);
  Eigen::VectorXd des_acc = pos_curve_->EvaluateSecondDerivative(current_time);
