...
// template<typename Vector3, template<class> class CovarianceBase =
// Kalman::StandardBase>
class PoseMeasurementModel
    : public Kalman::LinearizedMeasurementModel<State, PoseMeasurement,
                                                Kalman::StandardBase> {
public:
  ...
  void update_position_from_lfoot(const State &xhat,
                                  PoseMeasurement &measurementToUpdate) {
    measurementToUpdate.base_pose_lfoot_x() =
        xhat.base_pos_x() - xhat.lfoot_pos_x();
    measurementToUpdate.base_pose_lfoot_y() =
        xhat.base_pos_y() - xhat.lfoot_pos_y();
    measurementToUpdate.base_pose_lfoot_z() =
        xhat.base_pos_z() - xhat.lfoot_pos_z();
  }
  ...
  void update_position_from_rfoot(const State &xhat,
                                  PoseMeasurement &measurementToUpdate) {
    measurementToUpdate.base_pose_rfoot_x() =
        xhat.base_pos_x() - xhat.rfoot_pos_x();
    measurementToUpdate.base_pose_rfoot_y() =
        xhat.base_pos_y() - xhat.rfoot_pos_y();
    measurementToUpdate.base_pose_rfoot_z() =
        xhat.base_pos_z() - xhat.rfoot_pos_z();
  }

  PoseMeasurement h(const State &x) const {
    PoseMeasurement y_next;

    y_next.base_pose_lfoot_x() =
        H(0, 0) * x.base_pos_x() + H(0, 6) * x.lfoot_pos_x();
    y_next.base_pose_lfoot_y() =
        H(1, 1) * x.base_pos_y() + H(1, 7) * x.lfoot_pos_y();
    y_next.base_pose_lfoot_z() =
        H(2, 2) * x.base_pos_z() + H(2, 8) * x.lfoot_pos_z();
    y_next.base_pose_rfoot_x() =
        H(3, 0) * x.base_pos_x() + H(3, 9) * x.rfoot_pos_x();
    y_next.base_pose_rfoot_y() =
        H(4, 1) * x.base_pos_y() + H(4, 10) * x.rfoot_pos_y();
    y_next.base_pose_rfoot_z() =
        H(5, 2) * x.base_pos_z() + H(5, 11) * x.rfoot_pos_z();
