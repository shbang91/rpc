...
// Kalman::StandardBase>
class FloatingBaseSystemModel
    : public Kalman::LinearizedSystemModel<State, Control,
                                           Kalman::StandardBase> {
public:
  ...
  void initialize(const double &delta_t, const Eigen::Vector3d &sigma_base_pos,
  ...
  State f(const State &x, const Control &u) const {
    ...
    x_next.base_pos_x() = F(0, 0) * x.base_pos_x() + F(0, 3) * x.base_vel_x() +
                          B(0, 0) * u.accel_meas_x();
    x_next.base_pos_y() = F(1, 1) * x.base_pos_y() + F(1, 4) * x.base_vel_y() +
                          B(1, 1) * u.accel_meas_y();
    x_next.base_pos_z() = F(2, 2) * x.base_pos_z() + F(2, 5) * x.base_vel_z() +
                          B(2, 2) * u.accel_meas_z();
