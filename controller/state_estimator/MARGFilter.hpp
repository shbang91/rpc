...
const float gyroMeasError = 3.14159265358979 * (0.3f / 180.0f); // gyroscope measurement error in rad/s (shown as 0.3 deg/s)
const float gyroMeasDrift = 3.14159265358979 * (0.0001f / 180.0f); // gyroscope measurement error in rad/s/s (shown as 0.2f deg/s/s)
...
public:
    ...
    void initialize(float q_w, float q_x, float q_y, float q_z);
