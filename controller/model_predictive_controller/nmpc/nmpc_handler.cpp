...
NMPCHandler::NMPCHandler(PinocchioRobotSystem *robot,
                         TCIContainer *tci_container, int lfoot_id,
                         int rfoot_id)
    : MPCHandler(robot), tci_container_(tci_container), lfoot_id_(lfoot_id),
      rfoot_id_(rfoot_id), first_visit_(true) {
  ...
  old_lf_force_.setZero(6);
  old_rf_force_.setZero(6);

  old_com_pos_.setZero(3);
  old_com_vel_.setZero(3);
  old_com_acc_.setZero(3);
  old_base_ori_.setZero(4);
  old_base_vel_.setZero(3);
  old_base_acc_.setZero(3);
  old_lf_pos_.setZero(3);
  old_lf_vel_.setZero(3);
  old_lf_acc_.setZero(3);
  old_rf_pos_.setZero(3);
  old_rf_vel_.setZero(3);
  old_rf_acc_.setZero(3);
...
void NMPCHandler::SetDCMPos(Eigen::Vector3d dcm) { dcm_ = dcm; }
...
void NMPCHandler::_generateDCMTrajectory() {
  ...
  // generate a DCM and a Com trajectory on n_nodes, assuming the ZMP is in the
  // center of each foot and the DCM terminates on the final foot
  ...
  for (int i = footstep_list.size() - 2; i > 0; i--) {
    Eigen::VectorXd new_dcm =
        footstep_list[i].GetPos() +
        (dcm_pts[i + 1] - footstep_list[i].GetPos()) / std::exp(omega * T_step);
    dcm_pts[i] = new_dcm;
  }
  ...
  for (int j = 1; j < dcm_pts.size() - 2; j += 2) {
    time = 0;
    for (int i = 0; i <= 10; i++) {
      dcm_trj.push_back(std::exp(omega * time) * dcm_pts[j] +
                        (1 - std::exp(omega * time)) *
                            footstep_list[j].GetPos());
      time += dt;
    }

    time = 0;
    for (int i = 11; i <= 21; i++) {
      dcm_trj.push_back(std::exp(omega * time) * dcm_pts[j + 1] +
                        (1 - std::exp(omega * time)) *
                            footstep_list[j + 1].GetPos());
      time += dt;
    }
  }
...
void NMPCHandler::_populateStepInPlace() {
  ...
  footstep_list = FootStep::GetInPlaceWalkFootStep(
      n_steps_, nominal_footwidth_, robot_side_first_, mid_footstep);
...
void NMPCHandler::_populateStepForward() {
  ...
  footstep_list = FootStep::GetFwdWalkFootStep(n_steps_, nominal_forward_step_,
                                               nominal_footwidth_,
                                               robot_side_first_, mid_footstep);
  ...
  // Assign iterators to keep the current foot position at the beginning of the
  // walk
...
void NMPCHandler::_populateSideWalk(bool left_side) {
  ...
  footstep_list = FootStep::GetStrafeFootStep(
      n_steps_, nominal_strafe_distance_, nominal_footwidth_, mid_foot_stance_);
...
void NMPCHandler::SetFootstepToPublish(const int count) {
  ...
  if (!footstep_list.empty()) {
    ...
    if (count_ % int(T_ / ctrl_dt_ + 0.5) == 0 &&
        footstep_list_index_ < int(footstep_list.size() - 2)) {
      std::cout << "COUNT A: " << count_ << std::endl;
      std::cout << "Taking footsteps from " << footstep_list_index_ << " to "
                << footstep_list_index_ + 3 << " in a vector size of "
                << footstep_list.size() << std::endl;
    ...
    } else if (count_ % int(T_ / ctrl_dt_ + 0.5) == 0 &&
               footstep_list_index_ >= int(footstep_list.size() - 2) &&
               footstep_list_index_ < int(footstep_list.size() - 1)) {
      std::cout << "COUNT B: " << count_ << std::endl;
      std::cout << "Taking footsteps from " << footstep_list_index_ << " to "
                << footstep_list_index_ + 1 << " in a vector size of "
                << footstep_list.size() << std::endl;
      init_it = footstep_list.begin() + footstep_list_index_;
    ...
    if (count_ % int(T_ / n_nodes_ / ctrl_dt_ + 0.5) == 0) {
      ...
      //                {
      //                    _updateStartingStance();
      //                    std::vector<FootStep> fs{left_foot_stance_,
      //                    right_foot_stance_}; footstep_to_publish_ = fs;
      //                }
      //            }
    }
  }
...
void NMPCHandler::_GetMPCOutputData() {
  ...
  ZeroCopyInputStream *raw_input =
      new ArrayInputStream(update_.data(), update_.size());
  ...
  if (temp_res.com_size() > 0) {
    ...
    auto old_left_foot =
        _ConvertFoot(draco_link::l_foot_contact, old_mpc_res_, 1);
    old_lf_force_ =
        _ConvertFootForces(std::get<0>(old_left_foot),
                           draco_link::l_foot_contact, old_mpc_res_, 1);

    auto old_right_foot =
        _ConvertFoot(draco_link::r_foot_contact, old_mpc_res_, 1);
    old_rf_force_ =
        _ConvertFootForces(std::get<0>(old_right_foot),
                           draco_link::r_foot_contact, old_mpc_res_, 1);

    old_com_pos_ << old_mpc_res_.com(1).x(), old_mpc_res_.com(1).y(),
        old_mpc_res_.com(1).z();
    old_com_vel_ << old_mpc_res_.com_vel(1).xdot(),
        old_mpc_res_.com_vel(1).ydot(), old_mpc_res_.com_vel(1).zdot();
    old_com_acc_ << old_mpc_res_.com_acc(1).xddot(),
        old_mpc_res_.com_acc(1).yddot(), old_mpc_res_.com_acc(1).zddot();

    old_base_ori_ << old_mpc_res_.ori(1).x(), old_mpc_res_.ori(1).y(),
        old_mpc_res_.ori(1).z(), old_mpc_res_.ori(1).w();
    old_base_vel_ << old_mpc_res_.omega(1).x(), old_mpc_res_.omega(1).y(),
        old_mpc_res_.omega(1).z();
    ...
    //        std::cout << "count_: " << count_ << " - from [" <<
    //        old_com_pos_.transpose() << "] to [" << mpc_res_.com(1).x() << ",
    //        " << mpc_res_.com(1).y() << ", " << mpc_res_.com(1).z() << "]" <<
    //        std::endl;

    auto right_foot_ref = _ConvertFoot(draco_link::r_foot_contact, mpc_res_, 1);
    auto new_rf_force = _ConvertFootForces(
        std::get<0>(right_foot_ref), draco_link::r_foot_contact, mpc_res_, 1);
  }
}

std::tuple<Eigen::Vector3d, Eigen::Vector3d, Eigen::Vector3d>
NMPCHandler::_ConvertFoot(int foot, HORIZON_TO_PNC::MPCResult mpc_res,
                          int index) {
  ...
  if (foot == draco_link::l_foot_contact) {
    ...
    throw std::runtime_error(
        "[NMPCHandler: convertFoot()]: Wrong link selected!");

  double avg_xdot = 0, avg_ydot = 0, avg_zdot = 0;
  double avg_xddot = 0, avg_yddot = 0, avg_zddot = 0;
  for (int i = 0; i < foot_pos.pos_size(); i++) {
    if (foot == draco_link::l_foot_contact) {
      // logger_->add("left_contact_" + std::to_string(i),
      // Eigen::Vector3d(foot_pos.pos(i).x(), foot_pos.pos(i).y(),
      // foot_pos.pos(i).z()));
    }
    pos(0) += foot_pos.pos(i).x();
    pos(1) += foot_pos.pos(i).y();
    pos(2) += foot_pos.pos(i).z();
    vel(0) += foot_vel.vel(i).xdot();
    vel(1) += foot_vel.vel(i).ydot();
    vel(2) += foot_vel.vel(i).zdot();
    acc(0) += foot_acc.acc(i).xddot();
    acc(1) += foot_acc.acc(i).yddot();
    acc(2) += foot_acc.acc(i).zddot();
  }
...
NMPCHandler::_ConvertFootForces(Eigen::Vector3d foot_center, int foot,
  ...
  if (foot == draco_link::l_foot_contact) {
    ...
    throw std::runtime_error(
        "[NMPCHandler: convertFootForces()]: Wrong link selected!");
  ...
  for (int i = 0; i < force_foot.force_size(); i++) {
    ...
    Eigen::Vector3d arm =
        ...
        foot_center;
    wrench.tail<3>() +=
        Eigen::Vector3d(force_foot.force(i).f_x(), force_foot.force(i).f_y(),
                        force_foot.force(i).f_z());
    wrench.head<3>() += arm.cross(Eigen::Vector3d(force_foot.force(i).f_x(),
                                                  force_foot.force(i).f_y(),
                                                  force_foot.force(i).f_z()));
  }
...
Eigen::VectorXd NMPCHandler::_LinearInterpolation(Eigen::VectorXd start,
                                                  Eigen::VectorXd goal) {
  int index = count_ % int(T_ / n_nodes_ / ctrl_dt_);
  Eigen::VectorXd res =
      start + (goal - start) / int(T_ / n_nodes_ / ctrl_dt_) * interp_count_;
...
bool NMPCHandler::UpdateDesired() {
  // Update com
  Eigen::Vector3d com_pos_ref = _LinearInterpolation(
      old_com_pos_, Eigen::Vector3d(mpc_res_.com(1).x(), mpc_res_.com(1).y(),
                                    mpc_res_.com(1).z()));
  Eigen::Vector3d com_vel_ref = _LinearInterpolation(
      old_com_vel_,
      Eigen::Vector3d(mpc_res_.com_vel(1).xdot(), mpc_res_.com_vel(1).ydot(),
                      mpc_res_.com_vel(1).zdot()));
  Eigen::Vector3d com_acc_ref = _LinearInterpolation(
      old_com_acc_,
      Eigen::Vector3d(mpc_res_.com_acc(1).xddot(), mpc_res_.com_acc(1).yddot(),
                      mpc_res_.com_acc(1).zddot()));
  tci_container_->task_map_["com_task"]->UpdateDesired(com_pos_ref, com_vel_ref,
                                                       com_acc_ref);

  // Update torso orientation
  //    tci_container_->task_map_["torso_ori_task"]->UpdateDesired(Eigen::Vector4d(mpc_res_.ori(1).x(),
  //    mpc_res_.ori(1).y(), mpc_res_.ori(1).z(), mpc_res_.ori(1).w()),
  //                                                               Eigen::Vector3d(mpc_res_.omega(1).x(),
  //                                                               mpc_res_.omega(1).y(),
  //                                                               mpc_res_.omega(1).z()),
  //                                                               Eigen::Vector3d(mpc_res_.omega_dot(1).x(),
  //                                                               mpc_res_.omega_dot(1).y(),
  //                                                               mpc_res_.omega_dot(1).z()));
  tci_container_->task_map_["torso_ori_task"]->UpdateDesired(
      Eigen::Vector4d(0, 0, 0, 1), Eigen::Vector3d(0, 0, 0),
      Eigen::Vector3d(0, 0, 0));

  // Update left foot
  auto left_foot_ref = _ConvertFoot(draco_link::l_foot_contact, mpc_res_, 1);
  Eigen::Vector3d lf_pos_ref =
      _LinearInterpolation(old_lf_pos_, std::get<0>(left_foot_ref));
  Eigen::Vector3d lf_vel_ref =
      _LinearInterpolation(old_lf_vel_, std::get<1>(left_foot_ref));
  Eigen::Vector3d lf_acc_ref =
      _LinearInterpolation(old_lf_acc_, std::get<2>(left_foot_ref));
  tci_container_->task_map_["lf_pos_task"]->UpdateDesired(
      lf_pos_ref, lf_vel_ref, lf_acc_ref);

  tci_container_->task_map_["lf_ori_task"]->UpdateDesired(
      Eigen::Vector4d(0, 0, 0, 1), Eigen::Vector3d(0, 0, 0),
      Eigen::Vector3d(0, 0, 0));

  auto left_force_ref = _ConvertFootForces(
      std::get<0>(left_foot_ref), draco_link::l_foot_contact, mpc_res_, 1);
  auto left_force_ref_interpolated =
      _LinearInterpolation(old_lf_force_, left_force_ref);
  ...
  tci_container_->force_task_map_["lf_reaction_force_task"]->UpdateDesired(
      left_force_ref_interpolated);

  // Update right foot
  auto right_foot_ref = _ConvertFoot(draco_link::r_foot_contact, mpc_res_, 1);
  Eigen::Vector3d rf_pos_ref =
      _LinearInterpolation(old_rf_pos_, std::get<0>(right_foot_ref));
  Eigen::Vector3d rf_vel_ref =
      _LinearInterpolation(old_rf_vel_, std::get<1>(right_foot_ref));
  Eigen::Vector3d rf_acc_ref =
      _LinearInterpolation(old_rf_acc_, std::get<2>(right_foot_ref));

  tci_container_->task_map_["rf_pos_task"]->UpdateDesired(
      rf_pos_ref, rf_vel_ref, rf_acc_ref);

  tci_container_->task_map_["rf_ori_task"]->UpdateDesired(
      Eigen::Vector4d(0, 0, 0, 1), Eigen::Vector3d(0, 0, 0),
      Eigen::Vector3d(0, 0, 0));

  auto right_force_ref = _ConvertFootForces(
      std::get<0>(right_foot_ref), draco_link::r_foot_contact, mpc_res_, 1);
  auto right_force_ref_interpolated =
      _LinearInterpolation(old_rf_force_, right_force_ref);
  ...
  tci_container_->force_task_map_["rf_reaction_force_task"]->UpdateDesired(
      right_force_ref_interpolated);

  // Add to logger
  // logger_->add("unfiltered_com_pos_ref", Eigen::Vector3d(mpc_res_.com(1).x(),
  // mpc_res_.com(1).y(), mpc_res_.com(1).z()));
  // logger_->add("unfiltered_com_vel_ref",
  // Eigen::Vector3d(mpc_res_.com_vel(1).xdot(), mpc_res_.com_vel(1).ydot(),
  // mpc_res_.com_vel(1).zdot())); logger_->add("unfiltered_com_acc_ref",
  // Eigen::Vector3d(mpc_res_.com_acc(1).xddot(), mpc_res_.com_acc(1).yddot(),
  // mpc_res_.com_acc(1).zddot())); logger_->add("com_pos_ref",
  // tci_container_->task_map_["com_task"]->DesiredPos());
  // logger_->add("com_vel_ref",
  // tci_container_->task_map_["com_task"]->DesiredVel());
  // logger_->add("com_acc_ref",
  // tci_container_->task_map_["com_task"]->DesiredAcc());
  // logger_->add("actual_com_pos", robot_->GetRobotComPos());
  // logger_->add("actual_com_vel", robot_->GetRobotComLinVel());
  // logger_->add("torso_ori_ref", Eigen::Vector4d(mpc_res_.ori(1).x(),
  // mpc_res_.ori(1).y(), mpc_res_.ori(1).z(), mpc_res_.ori(1).w()));
  // logger_->add("torso_vel", Eigen::Vector3d(mpc_res_.omega(1).x(),
  // mpc_res_.omega(1).y(), mpc_res_.omega(1).z()));
  ...
  // logger_->add("actual_lf_pos",
  // robot_->GetLinkIsometry(draco_link::l_foot_contact).translation());
  // logger_->add("unfiltered_left_force_ref", left_force_ref);
  // logger_->add("left_force_ref", _LinearInterpolation(old_lf_force_,
  // left_force_ref)); logger_->add("unfiltered_rf_pos_ref",
  // std::get<0>(right_foot_ref)); logger_->add("unfiltered_rf_vel_ref",
  // std::get<1>(right_foot_ref)); logger_->add("unfiltered_rf_acc_ref",
  // std::get<2>(right_foot_ref)); logger_->add("rf_pos_ref", rf_pos_ref);
  // logger_->add("rf_vel_ref", rf_vel_ref);
  // logger_->add("rf_acc_ref", rf_acc_ref);
  // logger_->add("actual_rf_pos",
  // robot_->GetLinkIsometry(draco_link::r_foot_contact).translation());
  // logger_->add("unfiltered_right_force_ref", right_force_ref);
  // logger_->add("right_force_ref", _LinearInterpolation(old_rf_force_,
  // right_force_ref)); logger_->add("time", count_ * ctrl_dt_);
