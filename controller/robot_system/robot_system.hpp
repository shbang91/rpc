...
public:
  ...
  EigenVectorToMap(const Eigen::VectorXd &_joint_cmd) = 0;
  virtual Eigen::VectorXd
  MapToEigenVector(std::map<std::string, double> _joint_map) = 0;
  ...
  UpdateRobotModel(const Eigen::Vector3d &_base_com_pos,
                   ...
                   const std::map<std::string, double> _joint_positions,
                   const std::map<std::string, double> _joint_velocities,
  ...
  GetLinkVel(const std::string &_link_name) = 0;
  virtual Eigen::Matrix<double, 6, Eigen::Dynamic>
  GetLinkJacobian(const std::string &_link_name) = 0;
  virtual Eigen::Matrix<double, 6, 1>
  GetLinkJacobianDotQdot(const std::string &_link_name) = 0;
