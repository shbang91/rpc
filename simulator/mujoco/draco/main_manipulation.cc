...
// gripper
std::vector<std::string> gripper_jnt_vec_ = {
    "left_ezgripper_knuckle_palm_L1_1", "right_ezgripper_knuckle_palm_L1_1"};
std::vector<std::string> gripper_act_vec_ = {
    "left_ezgripper_knuckle_palm_L1_1", "right_ezgripper_knuckle_palm_L1_1"};

std::unordered_map<std::string, int> mj_gripper_qpos_map_;
std::unordered_map<std::string, int> mj_gripper_qvel_map_;
std::unordered_map<std::string, int> mj_gripper_act_map_;
...
std::vector<double> gripper_kp_;
std::vector<double> gripper_kd_;
...
void SetGripperJointAndActuatorMaps(
    mjModel *m, std::unordered_map<std::string, int> &mj_gripper_qpos_map,
    std::unordered_map<std::string, int> &mj_gripper_qvel_map,
    std::unordered_map<std::string, int> &mj_gripper_actuator_map,
    const std::vector<std::string> &gripper_joint_vec,
    const std::vector<std::string> &gripper_actuator_vec) {
  // create mujoco joint map
  for (const auto &joint_name : gripper_joint_vec) {
    int jnt_idx = mj_name2id(m, mjOBJ_JOINT, joint_name.c_str());
    if (jnt_idx == -1) {
      std::cout << "[MuJoCo Utils] Can't find the joint name: " << joint_name
                << " in MuJoCo model" << '\n';
    }
    mj_gripper_qpos_map[joint_name] = m->jnt_qposadr[jnt_idx];
    mj_gripper_qvel_map[joint_name] = m->jnt_dofadr[jnt_idx];
  }
  // create mujoco actuator map
  for (const auto &act_name : gripper_actuator_vec) {
    int act_idx = mj_name2id(m, mjOBJ_ACTUATOR, act_name.c_str());
    if (act_idx == -1) {
      std::cout << "[MuJoCo Utils] Can't find the actuator name: " << act_name
                << " in MuJoCo model" << '\n';
    }
    mj_gripper_actuator_map[act_name] = act_idx;
  }
}
...
void SetActuatorGains(
    const std::unordered_map<std::string, int> &mj_act_map,
    const std::unordered_map<std::string, int> &mj_gripper_act_map) {
  kp_.clear();
  kp_.resize(mj_act_map.size() + mj_gripper_act_map.size());
  kd_.clear();
  kd_.resize(mj_act_map.size() + mj_gripper_act_map.size());
  ...
  try {
    ...
    for (const auto &[mj_act_name, mj_act_idx] : mj_gripper_act_map) {
      util::ReadParameter(cfg_["actuator_gains"][mj_act_name], "kp", tmp);
      kp_[mj_act_idx] = tmp;
      util::ReadParameter(cfg_["actuator_gains"][mj_act_name], "kd", tmp);
      kd_[mj_act_idx] = tmp;
...
void SetInitialConfig(
    ...
    const std::unordered_map<std::string, int> &mj_gripper_qpos_map) {
  try {
    ...
    // set gripper joint pos
    for (const auto &[joint_name, qpos_adr] : mj_gripper_qpos_map) {
      d->qpos[qpos_adr] =
          util::ReadParameter<double>(cfg_["initial_config"], joint_name);
    }
  } catch (const std::runtime_error &ex) {
...
void CopyCommand() {
  ...
  for (const auto &[mj_act_name, mj_act_idx] : mj_act_map_) {
    ...
    int mj_qpos_idx = mj_qpos_map_.at(
        mj_act_name); // TODO: only valid if joint name = actuator name
    int mj_qvel_idx = mj_qvel_map_.at(
        mj_act_name); // TODO: only valid if joint name = actuator name
  ...
  // gripper joint impedance control law
  for (const auto &[mj_act_name, mj_act_idx] : mj_gripper_act_map_) {
    // int pin_act_idx = pin_act_map_.at(mj_act_name);
...
void PhysicsThread(mj::Simulate *sim, const char *filename) {
  ...
  if (filename != nullptr) {
    ...
    if (d) {
      ...
      // TODO: make this function mujoco_utils (static method)
      SetJointAndActuatorMaps(m, mj_qpos_map_, mj_qvel_map_, mj_act_map_,
                              pin_jnt_map_, pin_act_map_);
      SetGripperJointAndActuatorMaps(m, mj_gripper_qpos_map_,
                                     mj_gripper_qvel_map_, mj_gripper_act_map_,
                                     gripper_jnt_vec_, gripper_act_vec_);
      // ********************************************
      ...
      // Set actuator gains
      //**********************************************
      SetActuatorGains(mj_act_map_, mj_gripper_act_map_);
      //**********************************************

      // Set initial configuration
      //**********************************************
      SetInitialConfig(m, d, mj_qpos_map_, mj_gripper_qpos_map_);
      //**********************************************
