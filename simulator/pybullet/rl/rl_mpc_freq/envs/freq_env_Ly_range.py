...
class DracoEnvMpcFreq_Ly_range(DracoEnv_v2):
    def __init__(self, mpc_freq, sim_dt, eval = None, burn_in: bool = False, reduced_obs_size: bool = False, render: bool = False) -> None:
        super().__init__(mpc_freq=mpc_freq, sim_dt=sim_dt, reduced_obs_size=reduced_obs_size, render=render)
    ...
    def penalise_outside_Lx_bounds(self):
        ...
        error = np.square(error)
        error = np.exp(-error)
        return error*self._w_penalise_excessive_Lx        
...
if __name__ == "__main__":
    env = DracoEnvMpcFreq_Ly_10(5, Config.CONTROLLER_DT, reduced_obs_size=True, render = True)
