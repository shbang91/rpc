...
if __name__ == "__main__":
    ...
    render = False
    env = DracoEnvMpcFreq_Ly_10(mpc_freq, sim_dt, reduced_obs_size=reduced_obs_size, render = render)

    monitor_env = Monitor(env)
    vec_env = DummyVecEnv([lambda: monitor_env])

    #MODEL EVALUATION
    eval_env = DracoEnvMpcFreq_Ly_10(mpc_freq, sim_dt,reduced_obs_size=reduced_obs_size, render = render)
    ...
    save_dir = str1 + f"Ly_10_n_steps8192_batch_1024" 
    load_dir = str1 + f"Ly_10_n_steps8192_batch_1024"
    load_path = os.path.join(model_dir, load_dir) 
    save_path = os.path.join(model_dir, save_dir)      
    ## train model
    if new_model:
        tensorboard_dir = cwd + "/rl_log/freq_env/ppo/Ly_10/"
