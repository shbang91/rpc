...
from simulator.pybullet.rl.rl_mpc_freq.envs.freq_env_tilted_ground_10_Ly_15 import DracoEnvMpcFreq_tilted_ground_10_Ly_15
from config.draco.pybullet_simulation import Config

model_dir = cwd + "/rl_model/freq_env/tilted_ground_10_Ly_15/PPO"
env_dir = cwd + "/rl_env/freq_env/tilted_ground_10_Ly_15/PPO"
#import tracemalloc
import argparse
import torch
...
if __name__ == "__main__":
    if not new_model:
        parser = argparse.ArgumentParser(
            description='Training script for your RL model')
        parser.add_argument(
            '--timesteps', type=str,
            help='FileToLoad')  # Default value is set as an example
    ...
    render = False
    env = DracoEnvMpcFreq_tilted_ground_10_Ly_15(
    ...
    #MODEL EVALUATION
    # eval_env = DracoEnvMpcFreq_tilted_ground_Ly_range(
    # mpc_freq, sim_dt, reduced_obs_size=reduced_obs_size, render=render)
    # disturbance=False)
    ...
    save_dir = str1 + f"Ly_15_tilted_10_downhill"
    load_dir = str1 + f"Ly_15_tilted_10_downhill"
    load_path = os.path.join(model_dir, load_dir)
    save_path = os.path.join(model_dir, save_dir)
    ## train model
    #policy_kwargs = { 'full_std': False}
    if new_model:
        tensorboard_dir = cwd + "/rl_log/freq_env/Ly_15_tilted_10_ground/"
        ...
        model = PPO(
            ...
            device='cpu'
        )  #policy_kwargs=dict(net_arch=[64,64, dict(vf=[], pi=[])]),

        a = model.get_parameters()

        a['policy']['action_net.weight'] = torch.zeros_like(
            a['policy']['action_net.weight'])
        a['policy']['action_net.bias'] = torch.zeros_like(
            a['policy']['action_net.bias'])
