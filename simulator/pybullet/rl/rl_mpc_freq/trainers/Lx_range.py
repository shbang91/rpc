...
from simulator.pybullet.rl.rl_mpc_freq.envs.freq_env_Lx_range import DracoEnvMpcFreq_Lx_range

from config.draco.pybullet_simulation import Config

model_dir = cwd + "/rl_model/freq_env/Lx_range/PPO"
env_dir = cwd + "/rl_env/freq_env/Lx_range/PPO"
#import tracemalloc
import argparse
import torch
new_model = False
...
if __name__ == "__main__":
    ...
    render = False
    env = DracoEnvMpcFreq_Lx_range(mpc_freq, sim_dt, reduced_obs_size=reduced_obs_size, render = render)

    monitor_env = Monitor(env)
    vec_env = DummyVecEnv([lambda: monitor_env])

    #MODEL EVALUATION
    # eval_env = DracoEnvMpcFreq_Lx_range(mpc_freq, sim_dt,reduced_obs_size=reduced_obs_size, render = render)
    ...
    if new_model:
        tensorboard_dir = cwd + "/rl_log/freq_env/Lx_range/"
