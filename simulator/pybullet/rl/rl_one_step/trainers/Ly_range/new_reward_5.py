...
from simulator.pybullet.rl.rl_one_step.envs.Ly_range_new_r5 import DracoEnvOneStepMpcRange_v5
...
if __name__ == "__main__":
    ...
    render = False
    env = DracoEnvOneStepMpcRange_v5(mpc_freq, sim_dt, reduced_obs_size=reduced_obs_size, render = render)

    monitor_env = Monitor(env)
    vec_env = DummyVecEnv([lambda: monitor_env])

    #MODEL EVALUATION
    eval_env = DracoEnvOneStepMpcRange_v5(mpc_freq, sim_dt,reduced_obs_size=reduced_obs_size, render = render)
    ...
    save_dir = str1 + f"Ly_range_reward_5" 
    load_dir = str1 + f"Ly_range_reward_5"
