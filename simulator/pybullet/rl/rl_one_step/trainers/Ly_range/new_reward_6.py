...
from simulator.pybullet.rl.rl_one_step.envs.Ly_range_new_r6 import DracoEnvOneStepMpcRange_v6
...
if __name__ == "__main__":
    ...
    render = False
    env = DracoEnvOneStepMpcRange_v6(mpc_freq, sim_dt, reduced_obs_size=reduced_obs_size, render = render)

    monitor_env = Monitor(env)
    vec_env = DummyVecEnv([lambda: monitor_env])

    #MODEL EVALUATION
    eval_env = DracoEnvOneStepMpcRange_v6(mpc_freq, sim_dt,reduced_obs_size=reduced_obs_size, render = render)
    ...
    save_dir = str1 + f"Ly_range_reward_6" 
    load_dir = str1 + f"Ly_range_reward_6"
