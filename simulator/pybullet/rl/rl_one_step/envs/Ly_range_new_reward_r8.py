...
class DracoEnvOneStepMpcRange_v8(DracoEnv_v2):
    ...
    def _compute_reward(self, wbc_obs, action, done):
        ...
        self.reward_info = np.array([reward, self._w_alive_bonus,  self.sim_Lx(), self.reward_tracking_com_Ly(),
    ...
    def sim_Lx(self):
        error = self._new_wbc_obs[7] + self._old_wbc_obs[7] - 2*self._old_wbc_obs[1]
...
if __name__ == "__main__":
    import math
    yaw = 20
    env = DracoEnvOneStepMpcRange_v7( 0, Config.CONTROLLER_DT, reduced_obs_size=False, render = True)
