...
class DracoEnv_tilted_plane_10_downhill(gym.Env):
    metadata = {
    ...
    def __init__(self,
        ...
        if (eval is None) and (disturbance):
            print(
                "DISTURBANCE MUST NOT BE USED WHILE TRAINING UNLESS DIST ENV")
            #raise Warning
        ...
        if (self.render):
            self.client.configureDebugVisualizer(
                self.client.COV_ENABLE_RENDERING, 0)
        ...
        ground = self.client.loadURDF(
            cwd + "/robot_model/ground/tilted_plane_2.urdf", useFixedBase=1)
        ...
        if (self.render):
            self.client.configureDebugVisualizer(
                self.client.COV_ENABLE_RENDERING, 1)
    ...
    def reset(self, seed: int = 0):  #creates env
        ...
        if (self._eval is None):
            ...
            dir_command = np.array(
                (self._eval[0], self._eval[1], self._eval[2]))
    ...
    def step(self, action):
        ...
        info = {"reward_components": self.reward_info, "is_success": success}
    ...
    def _debug_sensor_data(self):
        ...
        if np.isnan(base_com_quat).any() or np.isinf(base_com_quat).any():
            print("exit 1", "iter", self._low_iter, "big iter", self._iter,
                  base_com_pos, base_com_quat)
            done = True
            return True
