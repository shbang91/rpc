...
from scripts.data_converter import ObservationConverter, get_local_trajectories, joint_embedding, global_act_keys, global_des_keys, global_action_keys
            ...
            if name.endswith(".hdf5"):
                with h5py.File(os.path.join(root, name)) as demo_file:
                    ...
                    obs_converter = ObservationConverter(
                        include_actions=True, include_desired=True)
                    data = obs_converter.convert(demo_file)

                    # TODO: test if this creates the group
