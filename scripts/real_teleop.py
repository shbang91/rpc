...
def visualize(data_path):
    ...
    pose_ee_vector_prv = np.concatenate(
        [mat_tool_offset[:3, 3], util.rot_to_quat(mat_tool_offset[:3, :3])]
    )
    ...
    for num in range(length):
        ...
        if num == 0:
            ...
            (line_3,) = ax3.plot(
                pos_data_3[0, 0], pos_data_3[0, 1], pos_data_3[0, 2], color="blue"
            )
            # point_3, = ax3.plot([], [], [], 'go')
            axis_3 = []
            for j in range(3):
            ...
            (line_4,) = ax4.plot(
                pos_data_4[0, 0], pos_data_4[0, 1], pos_data_4[0, 2], color="blue"
            )
            # point_4, = ax4.plot([], [], [], 'go')
            axis_4 = []
            for j in range(3):
    ...
    video_writer = cv2.VideoWriter(
        save_path, cv2.VideoWriter_fourcc(*"mp4v"), 30, img_buffer[0].shape[1::-1]
    )
...
if __name__ == "__main__":
    ...
    server = DracoZMQServer(
        ip=TARGET_IP, pub_port=PUB_PORT, sub_port=SUB_PORT, verbose=True
    )
    server.start()
