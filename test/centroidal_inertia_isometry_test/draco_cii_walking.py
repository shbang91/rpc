...
if __name__ == 'main':
    ...
    robot = pb.loadURDF(cwd + '/robot_model/draco/draco_modified.urdf',
                        INITIAL_POS,
                        INITIAL_QUAT,
                        useFixedBase=False)
    ...
    nominal_base_iso = liegroup.RpToTrans(util.quat_to_rot(nominal_base_quat), nominal_base_pos)
    nominal_rf_iso = pybullet_util.get_link_iso(draco, link_id['r_foot_contact'])
    nominal_lf_iso = pybullet_util.get_link_iso(draco, link_id['l_foot_contact'])

    ## ik prep
    joint_screws_in_ee_at_home, ee_SE3_at_home = dict(), dict()
    open_chain_joints_name_dict, ee_links_name_dict, base_links_name_dict  = dict(), dict(), dict()

    leg_list = ['left_leg', 'right_leg']

    open_chain_joints_name_dict['left_leg'] = ['l_hip_ie', 'l_hip_aa', 'l_hip_fe','l_knee_fe_jp', 'l_knee_fe_jd', 'l_ankle_fe', 'l_ankle_ie'] 
    open_chain_joints_name_dict['right_leg'] = ['r_hip_ie', 'r_hip_aa', 'r_hip_fe','r_knee_fe_jp', 'r_knee_fe_jd', 'r_ankle_fe', 'r_ankle_ie'] 

    ee_links_name_dict['left_leg'] = 'l_foot_contact'
    ee_links_name_dict['right_leg'] = 'r_foot_contact'

    base_links_name_dict['left_leg'] = 'torso'
    base_links_name_dict['right_leg'] = 'torso'
    ...
    robot_system.update_system(nominal_sensor_data['base_joint_pos'],
                               ...
                               nominal_joint_pos, nominal_sensor_data['joint_vel'],
                               True)
    ...
    final_rf_pos = nominal_rf_iso[:3,3] + np.random.uniform(RFOOT_POS_DEV_LB, RFOOT_POS_DEV_UB)
    ...
    mid_rf_iso = interpolation.iso_interpolate(nominal_rf_iso, final_rf_iso, 0.5)
    mid_rf_iso[2,3] += swing_height
    mid_rf_vel = (final_rf_iso[:3, 3] - nominal_rf_iso[:3, 3]) / swing_time
    mid_rf_vel[2] = 0.

    final_base_iso = interpolation.iso_interpolate(nominal_lf_iso, final_rf_iso, 0.5)

    #### Create curves using one step boundary
    base_pos_curve = interpolation.HermiteCurveVec(nominal_base_iso[:3, 3], np.zeros(3), final_base_iso[:3, 3], np.zeros(3), swing_time)
    base_quat_curve = interpolation.HermiteCurveQuat(util.rot_to_quat(nominal_base_iso[:3,:3]), np.zeros(3), util.rot_to_quat(final_base_iso[:3,:3]), np.zeros(3), swing_time)

    rf_pos_curve_first_half = interpolation.HermiteCurveVec(nominal_rf_iso[:3,3], np.zeros(3), mid_rf_iso[:3,3], mid_rf_vel, swing_time/2.)
    rf_pos_curve_second_half = interpolation.HermiteCurveVec(mid_rf_iso[:3,3], mid_rf_vel, final_rf_iso[:3,3], np.zeros(3), swing_time/2.)

    rf_quat_curve = interpolation.HermiteCurveQuat(util.rot_to_quat(nominal_rf_iso[:3,:3]), np.zeros(3), util.rot_to_quat(final_rf_iso[:3,:3]), np.zeros(3), swing_time)
    ...
    for t in np.linspace(0., swing_time, num = NUM_NODE_PER_SWING):
        ...
        if t <= swing_time/2.:
            rf_pos = rf_pos_curve_first_half.evaluate(t-swing_time/2.)
        else:
        ...
        rf_initial_guess = np.array([nominal_joint_pos[joint_name] for joint_name in open_chain_joints_name_dict['right_leg']])
        rf_joint_pos_sol, rf_ik_success = robot_kinematics.IKinBody(joint_screws_in_ee_at_home['right_leg'],ee_SE3_at_home['right_leg'], des_T_base_rf, rf_initial_guess)
