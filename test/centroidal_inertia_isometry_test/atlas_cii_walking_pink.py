#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Atlas humanoid standing on two feet and reaching with a hand."""
...
if __name__ == "__main__":
    ...
    robot = pin.RobotWrapper.BuildFromURDF(
        cwd + '/robot_model/atlas/atlas_v3.urdf',
        cwd + '/robot_model/atlas',
        root_joint=pin.JointModelFreeFlyer())
    ...
    lknee_idx = robot.index("l_leg_kny")
    q0[NUM_FLOATING_BASE + lknee_idx] = np.pi / 2
    rknee_idx = robot.index("r_leg_kny")
    q0[NUM_FLOATING_BASE + rknee_idx] = np.pi / 2
    l_hip_fe_idx = robot.index("l_leg_hpy")
    q0[NUM_FLOATING_BASE + l_hip_fe_idx] = -np.pi / 4
    r_hip_fe_idx = robot.index("r_leg_hpy")
    q0[NUM_FLOATING_BASE + r_hip_fe_idx] = -np.pi / 4
    l_ankle_fe_idx = robot.index("l_leg_aky")
    q0[NUM_FLOATING_BASE + l_ankle_fe_idx] = -np.pi / 4
    r_ankle_fe_idx = robot.index("r_leg_aky")
    ...
    data_saver = DataSaver('atlas_cii_walking_pink.pkl')
    ...
    rfoot_contact_frame = viz.viewer["r_foot"]
