#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""iCub humanoid standing on two feet and reaching with a hand."""
...
## parameters
INITIAL_POS = [0., 0., 0.505]
INITIAL_QUAT = [0., 0., -1., 0.]
NUM_FLOATING_BASE = 5
NORMALIZE_FACTOR = 1.
...
if __name__ == "__main__":
    ...
    # cwd + '/robot_model/icub/icub_reduced.urdf',
    # cwd + '/robot_model/icub',
    # root_joint=pin.JointModelFreeFlyer())
    ...
    lknee_idx = robot.index("l_knee")
    q0[NUM_FLOATING_BASE + lknee_idx] = -np.pi / 2
    rknee_idx = robot.index("r_knee")
    q0[NUM_FLOATING_BASE + rknee_idx] = -np.pi / 2
    l_hip_fe_idx = robot.index("l_hip_pitch")
    q0[NUM_FLOATING_BASE + l_hip_fe_idx] = np.pi / 4
    r_hip_fe_idx = robot.index("r_hip_pitch")
    q0[NUM_FLOATING_BASE + r_hip_fe_idx] = np.pi / 4
    l_ankle_fe_idx = robot.index("l_ankle_pitch")
    q0[NUM_FLOATING_BASE + l_ankle_fe_idx] = -np.pi / 4
    r_ankle_fe_idx = robot.index("r_ankle_pitch")
    q0[NUM_FLOATING_BASE + r_ankle_fe_idx] = -np.pi / 4
    l_shoulder_aa_idx = robot.index("l_shoulder_roll")
    q0[NUM_FLOATING_BASE + l_shoulder_aa_idx] = np.pi / 2
    r_shoulder_aa_idx = robot.index("r_shoulder_roll")
    ...
    data_saver = DataSaver('icub_cii_walking_pink.pkl')
    ...
    rfoot_contact_frame = viz.viewer["r_sole"]
