#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Hubo humanoid standing on two feet and reaching with a hand."""
...
if __name__ == "__main__":
    ...
    robot = pin.RobotWrapper.BuildFromURDF(
        cwd + '/robot_model/hubo/URDF_HuboPlus_Float.urdf',
        cwd + '/robot_model/hubo',
        root_joint=pin.JointModelFreeFlyer())
    ...
    data_saver = DataSaver('hubo_cii_walking_pink.pkl')

    # Tasks initialization for IK
    left_foot_task = FrameTask(
        "l_foot_contact_frame",
        position_cost=1.0,
    ...
    tasks = [left_foot_task, pelvis_task, right_foot_task, posture_task]

    nominal_base_iso = configuration.get_transform_frame_to_world(
        "Body_Torso").copy()
    nominal_rf_iso = configuration.get_transform_frame_to_world(
        "r_foot_contact_frame").copy()
    nominal_lf_iso = configuration.get_transform_frame_to_world(
        "l_foot_contact_frame").copy()
    ...
    rfoot_contact_frame = viz.viewer["r_foot_contact_frame"]
    ...
    for one_step_x in np.linspace(ONE_STEP_X_LB, ONE_STEP_X_UB, num=10):
        for one_step_y in np.linspace(ONE_STEP_Y_LB, ONE_STEP_Y_UB, num=10):
            ...
            for t in np.linspace(0., swing_time, num=NUM_NODE_PER_SWING):
                ...
                pelvis_task.set_target(base_target_iso)
