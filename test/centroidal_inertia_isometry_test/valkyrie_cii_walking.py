...
def set_initial_config(robot, joint_id):
    pb.resetJointState(robot, joint_id["leftKneePitch"], np.pi / 2, 0.0)
    pb.resetJointState(robot, joint_id["rightKneePitch"], np.pi / 2, 0.0)

    pb.resetJointState(robot, joint_id["leftHipPitch"], -np.pi / 4, 0.0)
    pb.resetJointState(robot, joint_id["rightHipPitch"], -np.pi / 4, 0.0)

    pb.resetJointState(robot, joint_id["leftAnklePitch"], -np.pi / 4, 0.0)
    pb.resetJointState(robot, joint_id["rightAnklePitch"], -np.pi / 4, 0.0)
...
if __name__ == "__main__":
    ...
    leg_length = (
        pybullet_util.get_link_iso(robot, link_id["rightHipYawLink"])[2, 3]
        - pybullet_util.get_link_iso(robot, link_id["rightCOP_Frame"])[2, 3]
    )
