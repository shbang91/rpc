...
def set_initial_config(robot, joint_id):
    pb.resetJointState(robot, joint_id["LKP"], np.pi / 2, 0.0)
    pb.resetJointState(robot, joint_id["RKP"], np.pi / 2, 0.0)

    pb.resetJointState(robot, joint_id["LHP"], -np.pi / 4, 0.0)
    pb.resetJointState(robot, joint_id["RHP"], -np.pi / 4, 0.0)

    pb.resetJointState(robot, joint_id["LAP"], -np.pi / 4, 0.0)
    pb.resetJointState(robot, joint_id["RAP"], -np.pi / 4, 0.0)

    pb.resetJointState(robot, joint_id["LSR"], np.pi / 2, 0.0)
    pb.resetJointState(robot, joint_id["RSR"], -np.pi / 2, 0.0)
...
if __name__ == "__main__":
    ...
    open_chain_joints_name_dict["left_leg"] = ["LHY", "LHR", "LHP", "LKP", "LAP", "LAR"]
    ...
    leg_length = (
        pybullet_util.get_link_iso(robot, link_id["Body_Torso"])[2, 3]
        - pybullet_util.get_link_iso(robot, link_id["r_foot_contact_frame"])[2, 3]
    )
    ...
    robot_system = PinocchioRobotSystem(
        cwd + "/robot_model/hubo/URDF_HuboPlus_Float_2.urdf",
