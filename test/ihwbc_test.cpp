...
protected:
  IHWBCTest() {
    ...
    nominal_joint_pose << 0., 0.020, -0.785, 0.785, 0.785, -0.785, -0.020, 0.,
        0.523, 0., -1.57, 0., 0., 0., 0., -0.020, -0.785, 0.785, 0.785, -0.785,
        0.020, 0., -0.523, 0., -1.57, 0., 0.;
  ...
  void updateExpRobotConfiguration(PinocchioRobotSystem *_robot) {
    Eigen::Vector3d bjoint_pos(0.10707219690084457, -0.076886385679245,
                               0.7746838927268982);
    //      Eigen::Quaterniond bjoint_quat(0.003524184925481677,
    //      0.009243202395737171, 0.569104015827179, 0.8222060203552246);
    Eigen::Quaterniond bjoint_quat(0.8222060203552246, 0.003524184925481677,
                                   0.009243202395737171, 0.569104015827179);
    ...
    joint_pos << -0.004790077917277813, 0.029247676953673363,
        -0.6904628276824951, 0.699665904045105, 0.699665904045105,
        -0.7234567999839783, -0.06926488876342773, 0.011308951303362846,
        0.517723560333252, -0.027543433010578156, -1.526259183883667,
        -0.0018239645287394524, 0.03846093639731407, -0.020014101639389992,
        0.0008405676344409585, 0.01193675771355629, -0.7088733911514282,
        0.7380250692367554, 0.7380250692367554, -0.7637600898742676,
        -0.05901860445737839, 0.0012755452189594507, -0.5212283134460449,
        0.02195368893444538, -1.5299373865127563, -0.00794570054858923,
        0.02179921790957451;

    _robot->UpdateRobotModel(bjoint_pos, bjoint_quat.normalized(),
                             bjoint_lin_vel, bjoint_ang_vel, joint_pos,
                             joint_vel, true);

    init_com_pos << 0.118077099, -0.03382825106, 0.90020298957;
    target_com_pos << 0.098783828, -0.041007604,
        0.8999999; // z-component: 0.658
    init_torso_quat = bjoint_quat;
    target_torso_quat = Eigen::Quaterniond(0.8238, 0., 0., 0.5669); // TODO
                                                                    // check
  ...
  void updateSimRobotConfiguration(PinocchioRobotSystem *_robot) {
    ...
    // the following data was obtained from a typical sim after reaching the
    // balance state
    joint_pos << -0.0001907337864395231, 0.0020557641983032227,
        -0.6990141272544861, 0.7028496265411377, 0.7028495669364929,
        -0.7068191766738892, -0.002055233344435692, -2.8940004170863176e-08,
        0.5229998826980591, -9.891494556768521e-08, -1.5699745416641235,
        -2.6745562990981853e-06, 0.00013958911586087197, 2.7940632207901217e-07,
        0.0016240504337474704, 0.00027807941660284996, -0.6997817158699036,
        0.7028868794441223, 0.7028902769088745, -0.7068465352058411,
        -0.00027810910250991583, 1.867117305209831e-08, -0.5229998826980591,
        1.6460536045315166e-08, -1.5699776411056519, -3.938637291867053e-06,
        0.0001500540820416063;
  ...
  void updateTmDesiredValues(UpperBodyTrajetoryManager *_up_body_tm,
    ...
    _fb_tm->InitializeFloatingBaseInterpolation(
        init_com_pos, target_com_pos, init_torso_quat, target_torso_quat, 1.);
  ...
  void setDesiredForce(ForceTrajectoryManager *_lf_force_tm,
                       ...
                       Eigen::VectorXd &des_rforce, double duration) {
  ...
  void updateContactJacobians(TCIContainer *_tci) {
    int rf_dim(0);
    for (const auto &[contact_str, contact_ptr] : _tci->contact_map_) {
  ...
  void printTasksCurrentAndDesired(TCIContainer *_tci) {
    ...
    std::cout << "upper_body_task curr pos: "
              << _tci->task_map_["upper_body_task"]->CurrentPos().transpose()
              << std::endl;
    std::cout << "upper_body_task des pos: "
              << _tci->task_map_["upper_body_task"]->DesiredPos().transpose()
              << std::endl;
    std::cout << "torso_ori_task curr pos: "
              << _tci->task_map_["torso_ori_task"]->CurrentPos().transpose()
              << std::endl;
    std::cout << "torso_ori_task des pos: "
              << _tci->task_map_["torso_ori_task"]->DesiredPos().transpose()
              << std::endl;
    std::cout << "-----------------------------" << std::endl;
    std::cout << "com_xy_task curr pos: "
              << _tci->task_map_["com_xy_task"]->CurrentPos().transpose()
              << std::endl;
    std::cout << "com_xy_task des pos: "
              << _tci->task_map_["com_xy_task"]->DesiredPos().transpose()
              << std::endl;
    std::cout << "-----------------------------" << std::endl;
    std::cout << "com_z_task curr pos: "
              << _tci->task_map_["com_z_task"]->CurrentPos().transpose()
              << std::endl;
    std::cout << "com_z_task des pos: "
              << _tci->task_map_["com_z_task"]->DesiredPos().transpose()
              << std::endl;
    std::cout << "-----------------------------" << std::endl;
    std::cout << "lf_pos_task curr pos: "
              << _tci->task_map_["lf_pos_task"]->CurrentPos().transpose()
              << std::endl;
    std::cout << "lf_pos_task des pos: "
              << _tci->task_map_["lf_pos_task"]->DesiredPos().transpose()
              << std::endl;
    std::cout << "lf_ori_task curr pos: "
              << _tci->task_map_["lf_ori_task"]->CurrentPos().transpose()
              << std::endl;
    std::cout << "lf_ori_task des pos: "
              << _tci->task_map_["lf_ori_task"]->DesiredPos().transpose()
              << std::endl;
    std::cout << "rf_pos_task curr pos: "
              << _tci->task_map_["rf_pos_task"]->CurrentPos().transpose()
              << std::endl;
    std::cout << "rf_pos_task des pos: "
              << _tci->task_map_["rf_pos_task"]->DesiredPos().transpose()
              << std::endl;
    std::cout << "rf_ori_task curr pos: "
              << _tci->task_map_["rf_ori_task"]->CurrentPos().transpose()
              << std::endl;
    std::cout << "rf_ori_task des pos: "
              << _tci->task_map_["rf_ori_task"]->DesiredPos().transpose()
              << std::endl;
  }
...
TEST_F(IHWBCTest, expBalanceUsingEvenGRFs) {
  ...
  // set tasks as used when in double_support_balance state (corresponding to
  // joint data used) (set Desired)
  ...
  double duration = 1.0;
  setDesiredForce(lf_force_tm, init_force, des_force, rf_force_tm, init_force,
                  des_force, duration);
  updateTmDesiredValues(upper_body_tm, floating_base_tm, lf_SE3_tm, rf_SE3_tm,
                        lf_force_tm, rf_force_tm);
  //  dcm_tm->InitializeParameters(cfg["dcm_walking"]);
  //  dcm_tm->Initialize(0., dcm_transfer_type::kInitial, init_torso_quat,
  //  init_dcm, init_dcm_vel); dcm_tm->UpdateDesired(1.0);
  ...
  ihwbc.ComputeTaskCosts(tci_container->task_map_,
                         tci_container->force_task_map_,
  ...
  std::cout
      << "Cmd RF (lfoot): "
      << tci_container->force_task_map_["lf_force_task"]->CmdRf().transpose()
      << std::endl;
  std::cout
      << "Cmd RF (rfoot): "
      << tci_container->force_task_map_["rf_force_task"]->CmdRf().transpose()
      << std::endl;
  std::cout << "Des RF (lfoot): "
            << tci_container->force_task_map_["lf_force_task"]
                   ->DesiredRf()
                   .transpose()
            << std::endl;
  std::cout << "Des RF (rfoot): "
            << tci_container->force_task_map_["rf_force_task"]
                   ->DesiredRf()
                   .transpose()
            << std::endl;
  for (const auto &[task_str, task_ptr] :
       tci_container->task_unweighted_cost_map_) {
    std::cout << task_str
              << "_cost: " << tci_container->task_unweighted_cost_map_[task_str]
              << std::endl;
    std::cout << task_str << "_weighted_cost: "
              << tci_container->task_weighted_cost_map_[task_str] << std::endl;
  }
}
