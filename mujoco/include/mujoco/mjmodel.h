...
typedef enum mjtDyn_ { // type of actuator dynamics
  ...
  mjDYN_FILTEREXACT,   // linear filter: da/dt = (u-a) / tau, with exact
                       // integration
...
typedef enum mjtSensor_ { // type of sensor
  ...
  // sensors attached to an object with spatial frame: (x)body, geom, site,
  // camera
  ...
  // sensors related to kinematic subtrees; attached to a body (which is the
  // subtree root)
...
struct mjVisual_ {        // visualization options
  struct {                // global parameters
    ...
    int ellipsoidinertia; // geom for inertia visualization (0: box, 1:
                          // ellipsoid)
...
struct mjModel_ {
  ...
  mjtNum *body_user;     // user data                                (nbody x
                         // nuser_body)
  ...
  mjtNum *geom_user; // user data                                (ngeom x
                     // nuser_geom)
  ...
  mjtNum *site_user;   // user data                                (nsite x
                       // nuser_site)
  ...
  int *light_targetbodyid; // id of targeted body; -1: none            (nlight x
                           // 1)
  ...
  float *light_attenuation; // OpenGL attenuation (quadratic model)     (nlight
                            // x 3)
  ...
  int *flex_vertbodyid; // vertex body ids                          (nflexvert x
                        // 1)
  int *flex_edge; // edge vertex ids (2 per edge)             (nflexedge x 2)
  int *
      flex_elem; // element vertex ids (dim+1 per elem)      (nflexelemdata x 1)
  int *flex_elemlayer; // element distance from surface, 3D only   (nflexelem x
                       // 1)
  int *flex_shell; // shell fragment vertex ids (dim per frag) (nflexshelldata x
                   // 1)
  int *
  ...
  mjtNum *flex_edgestiffness; // edge stiffness                           (nflex
                              // x 1)
  mjtNum
  ...
  mjtByte *flexedge_rigid; // are both edge vertices in same body (nflexedge x
                           // 1)
  mjtByte
  ...
  int *mesh_facenormal; // normal face data                         (nmeshface x
                        // 3)
  int *mesh_facetexcoord; // texture face data                        (nmeshface
                          // x 3)
  ...
  float *skin_texcoord; // vertex texcoords for all skin meshes (nskintexvert x
                        // 2)
  int *skin_face; // triangle faces for all skin meshes       (nskinface x 3)
  int *skin_bonevertadr; // first vertex in each bone                (nskinbone
                         // x 1)
  int *skin_bonevertnum; // number of vertices in each bone          (nskinbone
                         // x 1)
  float *skin_bonebindpos;  // bind pos of each bone (nskinbone x 3)
  float *skin_bonebindquat; // bind quat of each bone (nskinbone x 4)
  int *skin_bonebodyid; // body id of each bone                     (nskinbone x
                        // 1)
  ...
  float *hfield_data; // elevation data                             (nhfielddata
                      // x 1)
  int *hfield_pathadr; // address of asset path for hfield; -1: none (nhfield x
                       // 1)
  ...
  mjtNum *pair_solreffriction; // solver reference: contact friction (npair x
                               // mjNREF)
  mjtNum
  ...
  int *exclude_signature; // body1 << 16 + body2                      (nexclude
                          // x 1)
  ...
  mjtNum *tendon_solref_lim; // constraint solver reference: limit (ntendon x
                             // mjNREF)
  mjtNum *tendon_solimp_lim; // constraint solver impedance: limit (ntendon x
                             // mjNIMP)
  mjtNum *tendon_solref_fri; // constraint solver reference: friction (ntendon x
                             // mjNREF)
  mjtNum *tendon_solimp_fri; // constraint solver impedance: friction (ntendon x
                             // mjNIMP)
  mjtNum
      *tendon_range; // tendon length limits                     (ntendon x 2)
  mjtNum
      *tendon_margin; // min distance for limit detection         (ntendon x 1)
  mjtNum *tendon_stiffness; // stiffness coefficient                    (ntendon
                            // x 1)
  mjtNum
  ...
  mjtNum *tendon_user; // user data                                (ntendon x
                       // nuser_tendon)
  ...
  mjtNum *actuator_gainprm; // gain parameters                          (nu x
                            // mjNGAIN)
  mjtNum *actuator_biasprm; // bias parameters                          (nu x
                            // mjNBIAS)
  mjtByte
  ...
  mjtNum *actuator_user;    // user data                                (nu x
                            // nuser_actuator)
  ...
  int *sensor_needstage; // required compute stage (mjtStage)        (nsensor x
                         // 1)
  ...
  mjtNum *sensor_user; // user data                                (nsensor x
                       // nuser_sensor)
  ...
  int *name_excludeadr; // exclude name pointers                    (nexclude x
                        // 1)
  ...
  int *name_numericadr; // numeric name pointers                    (nnumeric x
                        // 1)
