...
struct mjData_ {
  ...
  size_t maxuse_threadstack[mjMAXTHREAD]; // maximum stack allocation per thread
                                          // in bytes
  ...
  int solver_nnz[mjNISLAND]; // number of non-zeros in Hessian or efc_AR, per
                             // island
  ...
  int *wrap_obj; // geom id; -1: site; -2: pulley                    (nwrap*2 x
                 // 1)
  ...
  mjtNum *qLDiagSqrtInv; // 1/sqrt(diag(D))                                  (nv
                         // x 1)
  ...
  mjtNum *qfrc_inverse; // net external force; should equal:                (nv
                        // x 1) qfrc_applied + J'*xfrc_applied + qfrc_actuator
  ...
  int *efc_J_rowsuper; // number of subsequent rows in supernode           (nefc
                       // x 1)
  int *efc_J_colind; // column indices in constraint Jacobian            (nnzJ x
                     // 1)
  int *efc_JT_rownnz; // number of non-zeros in constraint Jacobian row T (nv x
                      // 1)
  int *efc_JT_rowadr; // row start address in colind array              T (nv x
                      // 1)
  int *efc_JT_rowsuper; // number of subsequent rows in supernode         T (nv
                        // x 1)
  int *efc_JT_colind; // column indices in constraint Jacobian          T (nnzJ
                      // x 1)
  ...
  int *island_dofind; // island dof indices; -1: none                     (nv x
                      // 1)
  int *dof_islandind; // dof island indices; -1: none                     (nv x
                      // 1)
  int *
  ...
  int *island_efcind; // island constraint indices                        (nefc
                      // x 1)
  ...
  int *efc_AR_rowadr; // row start address in colind array                (nefc
                      // x 1)
  int *efc_AR_colind; // column indices in sparse AR                      (nefc
                      // x nefc)
  mjtNum
