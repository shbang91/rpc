...
// Add file to VFS, return 0: success, 1: full, 2: repeated name, -1: failed to
// load.
MJAPI int mj_addFileVFS(mjVFS *vfs, const char *directory,
                        const char *filename);

// Add file to VFS from buffer, return 0: success, 1: full, 2: repeated name,
// -1: failed to load.
MJAPI int mj_addBufferVFS(mjVFS *vfs, const char *name, const void *buffer,
                          int nbuffer);
...
MJAPI mjModel *mj_loadXML(const char *filename, const mjVFS *vfs, char *error,
                          int error_sz);

// Update XML data structures with info from low-level model, save as MJCF.
// If error is not NULL, it must have size error_sz.
MJAPI int mj_saveLastXML(const char *filename, const mjModel *m, char *error,
                         int error_sz);
...
// Advance simulation, use control callback to obtain external force and
// control.
...
MJAPI void mj_forwardSkip(const mjModel *m, mjData *d, int skipstage,
                          int skipsensor);

// Inverse dynamics with skip; skipstage is mjtStage.
MJAPI void mj_inverseSkip(const mjModel *m, mjData *d, int skipstage,
                          int skipsensor);
...
// given.
MJAPI void mj_saveModel(const mjModel *m, const char *filename, void *buffer,
                        int buffer_sz);
...
MJAPI void mj_resetDataDebug(const mjModel *m, mjData *d,
...
// Free the current mjData stack frame. All pointers returned by mj_stackAlloc
// since the last call to mj_markStack must no longer be used afterwards.
...
// float_format must be a valid printf-style format string for a single float
// value.
...
// float_format must be a valid printf-style format string for a single float
// value
MJAPI void mj_printFormattedData(const mjModel *m, mjData *d,
                                 const char *filename,
                                 const char *float_format);
...
MJAPI void mju_printMatSparse(const mjtNum *mat, int nr, const int *rownnz,
...
MJAPI void mj_solveM(const mjModel *m, mjData *d, mjtNum *x, const mjtNum *y,
                     int n);
...
// Compute qfrc_passive from spring-dampers, gravity compensation and fluid
// forces.
MJAPI void mj_passive(const mjModel *m, mjData *d);

// Sub-tree linear velocity and angular momentum: compute subtree_linvel,
// subtree_angmom.
...
// Compute efc_state, efc_force, qfrc_constraint, and (optionally) cone
// Hessians. If cost is not NULL, set *cost = s(jar) where jar = Jac*qacc-aref.
...
// Get state.
MJAPI void mj_getState(const mjModel *m, const mjData *d, mjtNum *state,
                       unsigned int spec);

// Set state.
MJAPI void mj_setState(const mjModel *m, mjData *d, const mjtNum *state,
                       unsigned int spec);
...
MJAPI void mj_mulJacVec(const mjModel *m, const mjData *d, mjtNum *res,
                        const mjtNum *vec);

// Multiply dense or sparse constraint Jacobian transpose by vector.
MJAPI void mj_mulJacTVec(const mjModel *m, const mjData *d, mjtNum *res,
                         const mjtNum *vec);

// Compute 3/6-by-nv end-effector Jacobian of global point attached to given
// body.
MJAPI void mj_jac(const mjModel *m, const mjData *d, mjtNum *jacp, mjtNum *jacr,
                  const mjtNum point[3], int body);

// Compute body frame end-effector Jacobian.
MJAPI void mj_jacBody(const mjModel *m, const mjData *d, mjtNum *jacp,
                      mjtNum *jacr, int body);

// Compute body center-of-mass end-effector Jacobian.
MJAPI void mj_jacBodyCom(const mjModel *m, const mjData *d, mjtNum *jacp,
                         mjtNum *jacr, int body);

// Compute subtree center-of-mass end-effector Jacobian.
MJAPI void mj_jacSubtreeCom(const mjModel *m, mjData *d, mjtNum *jacp,
                            int body);

// Compute geom end-effector Jacobian.
MJAPI void mj_jacGeom(const mjModel *m, const mjData *d, mjtNum *jacp,
                      mjtNum *jacr, int geom);

// Compute site end-effector Jacobian.
MJAPI void mj_jacSite(const mjModel *m, const mjData *d, mjtNum *jacp,
                      mjtNum *jacr, int site);

// Compute translation end-effector Jacobian of point, and rotation Jacobian of
// axis.
MJAPI void mj_jacPointAxis(const mjModel *m, mjData *d, mjtNum *jacPoint,
                           mjtNum *jacAxis, const mjtNum point[3],
                           const mjtNum axis[3], int body);

// Compute subtree angular momentum matrix.
MJAPI void mj_angmomMat(const mjModel *m, mjData *d, mjtNum *mat, int body);

// Get id of object with the specified mjtObj type and name, returns -1 if id
// not found.
MJAPI int mj_name2id(const mjModel *m, int type, const char *name);

// Get name of object with the specified mjtObj type and id, returns NULL if
// name not found.
...
MJAPI void mj_mulM(const mjModel *m, const mjData *d, mjtNum *res,
                   const mjtNum *vec);

// Multiply vector by (inertia matrix)^(1/2).
MJAPI void mj_mulM2(const mjModel *m, const mjData *d, mjtNum *res,
                    const mjtNum *vec);

// Add inertia matrix to destination matrix.
// Destination can be sparse uncompressed, or dense when all int* are NULL
MJAPI void mj_addM(const mjModel *m, mjData *d, mjtNum *dst, int *rownnz,
                   int *rowadr, int *colind);

// Apply Cartesian force and torque (outside xfrc_applied mechanism).
MJAPI void mj_applyFT(const mjModel *m, mjData *d, const mjtNum force[3],
                      const mjtNum torque[3], const mjtNum point[3], int body,
                      mjtNum *qfrc_target);

// Compute object 6D velocity (rot:lin) in object-centered frame, world/local
// orientation.
MJAPI void mj_objectVelocity(const mjModel *m, const mjData *d, int objtype,
                             int objid, mjtNum res[6], int flg_local);

// Compute object 6D acceleration (rot:lin) in object-centered frame,
// world/local orientation.
MJAPI void mj_objectAcceleration(const mjModel *m, const mjData *d, int objtype,
                                 int objid, mjtNum res[6], int flg_local);

// Extract 6D force:torque given contact id, in the contact frame.
MJAPI void mj_contactForce(const mjModel *m, const mjData *d, int id,
                           mjtNum result[6]);
...
MJAPI void mj_integratePos(const mjModel *m, mjtNum *qpos, const mjtNum *qvel,
                           mjtNum dt);
...
MJAPI void mj_local2Global(mjData *d, mjtNum xpos[3], mjtNum xmat[9],
                           const mjtNum pos[3], const mjtNum quat[4], int body,
                           mjtByte sameframe);
...
MJAPI const char *mj_getPluginConfig(const mjModel *m, int plugin_id,
                                     const char *attrib);

// Load a dynamic library. The dynamic library is assumed to register one or
// more plugins.
MJAPI void mj_loadPluginLibrary(const char *path);

// Scan a directory and load all dynamic libraries. Dynamic libraries in the
// specified directory are assumed to register one or more plugins. Optionally,
// if a callback is specified, it is called for each dynamic library encountered
// that registers plugins.
MJAPI void mj_loadAllPluginLibraries(const char *directory,
...
MJAPI void mj_multiRay(const mjModel *m, mjData *d, const mjtNum pnt[3],
                       const mjtNum *vec, const mjtByte *geomgroup,
                       mjtByte flg_static, int bodyexclude, int *geomid,
                       mjtNum *dist, int nray, mjtNum cutoff);

// Intersect ray (pnt+x*vec, x>=0) with visible geoms, except geoms in
// bodyexclude. Return distance (x) to nearest surface, or -1 if no intersection
// and output geomid. geomgroup, flg_static are as in mjvOption; geomgroup==NULL
// skips group exclusion.
MJAPI mjtNum mj_ray(const mjModel *m, const mjData *d, const mjtNum pnt[3],
...
// Intersect ray with pure geom, return nearest distance or -1 if no
// intersection.
MJAPI mjtNum mju_rayGeom(const mjtNum pos[3], const mjtNum mat[9],
...
MJAPI mjtNum mju_rayFlex(const mjModel *m, const mjData *d, int flex_layer,
                         mjtByte flg_vert, mjtByte flg_edge, mjtByte flg_face,
                         mjtByte flg_skin, int flexid, const mjtNum *pnt,
                         const mjtNum *vec, int vertid[1]);

// Intersect ray with skin, return nearest distance or -1 if no intersection,
// and also output nearest vertex id.
MJAPI mjtNum mju_raySkin(int nface, int nvert, const int *face,
...
MJAPI void mjv_room2model(mjtNum modelpos[3], mjtNum modelquat[4],
                          const mjtNum roompos[3], const mjtNum roomquat[4],
                          const mjvScene *scn);

// Transform pose from model to room space.
MJAPI void mjv_model2room(mjtNum roompos[3], mjtNum roomquat[4],
                          const mjtNum modelpos[3], const mjtNum modelquat[4],
                          const mjvScene *scn);
...
// Get frustum height at unit distance from camera; average left and right
// OpenGL cameras.
MJAPI mjtNum mjv_frustumHeight(const mjvScene *scn);

// Rotate 3D vec in horizontal plane by angle between (0,1) and
// (forward_x,forward_y).
MJAPI void mjv_alignToCamera(mjtNum res[3], const mjtNum vec[3],
                             const mjtNum forward[3]);

// Move camera with mouse; action is mjtMouse.
MJAPI void mjv_moveCamera(const mjModel *m, int action, mjtNum reldx,
                          mjtNum reldy, const mjvScene *scn, mjvCamera *cam);
...
MJAPI void mjv_movePerturb(const mjModel *m, const mjData *d, int action,
                           mjtNum reldx, mjtNum reldy, const mjvScene *scn,
                           mjvPerturb *pert);
...
MJAPI void mjv_moveModel(const mjModel *m, int action, mjtNum reldx,
                         mjtNum reldy, const mjtNum roomup[3], mjvScene *scn);

// Copy perturb pos,quat from selected body; set scale for perturbation.
MJAPI void mjv_initPerturb(const mjModel *m, mjData *d, const mjvScene *scn,
                           mjvPerturb *pert);

// Set perturb pos,quat in d->mocap when selected body is mocap, and in d->qpos
// otherwise. Write d->qpos only if flg_paused and subtree root for selected
// body has free joint.
MJAPI void mjv_applyPerturbPose(const mjModel *m, mjData *d,
                                const mjvPerturb *pert, int flg_paused);

// Set perturb force,torque in d->xfrc_applied, if selected body is dynamic.
MJAPI void mjv_applyPerturbForce(const mjModel *m, mjData *d,
                                 const mjvPerturb *pert);

// Return the average of two OpenGL cameras.
MJAPI mjvGLCamera mjv_averageCamera(const mjvGLCamera *cam1,
                                    const mjvGLCamera *cam2);
...
MJAPI void mjv_makeConnector(mjvGeom *geom, int type, mjtNum width, mjtNum a0,
                             mjtNum a1, mjtNum a2, mjtNum b0, mjtNum b1,
                             mjtNum b2);
...
// Update entire scene from a scene state, return the number of new
// mjWARN_VGEOMFULL warnings.
MJAPI int mjv_updateSceneFromState(const mjvSceneState *scnstate,
                                   const mjvOption *opt, const mjvPerturb *pert,
...
MJAPI void mjv_updateSceneState(const mjModel *m, mjData *d,
                                const mjvOption *opt, mjvSceneState *scnstate);
...
// Update camera.
MJAPI void mjv_updateCamera(const mjModel *m, mjData *d, mjvCamera *cam,
                            mjvScene *scn);
...
MJAPI void mjr_uploadTexture(const mjModel *m, const mjrContext *con,
                             int texid);
...
MJAPI void mjr_uploadHField(const mjModel *m, const mjrContext *con,
                            int hfieldid);
...
// If only one buffer is available, set that buffer and ignore framebuffer
// argument.
...
MJAPI void mjr_blitBuffer(mjrRect src, mjrRect dst, int flg_color,
                          int flg_depth, const mjrContext *con);
...
MJAPI void mjr_overlay(int font, int gridpos, mjrRect viewport,
                       const char *overlay, const char *overlay2,
                       const mjrContext *con);
...
MJAPI void mjr_label(mjrRect viewport, int font, const char *txt, float r,
                     float g, float b, float a, float rt, float gt, float bt,
                     const mjrContext *con);
...
MJAPI void mju_addScl3(mjtNum res[3], const mjtNum vec1[3],
                       const mjtNum vec2[3], mjtNum scl);
...
MJAPI void mju_rotVecMat(mjtNum res[3], const mjtNum vec[3],
                         const mjtNum mat[9]);

// Multiply vector by transposed 3D rotation matrix: res = mat' * vec.
MJAPI void mju_rotVecMatT(mjtNum res[3], const mjtNum vec[3],
                          const mjtNum mat[9]);
...
// Multiply square matrix with vectors on both sides: returns vec1' * mat *
// vec2.
...
// Set res = mat' * diag * mat if diag is not NULL, and res = mat' * mat
// otherwise.
MJAPI void mju_sqrMatTD(mjtNum *res, const mjtNum *mat, const mjtNum *diag,
                        int nr, int nc);

// Coordinate transform of 6D motion or force vector in rotation:translation
// format. rotnew2old is 3-by-3, NULL means no rotation; flg_force specifies
// force or motion type.
...
MJAPI void mju_derivQuat(mjtNum res[4], const mjtNum quat[4],
                         const mjtNum vel[3]);
...
// seq[0,1,2] must be in 'xyzXYZ', lower/upper-case mean intrinsic/extrinsic
// rotations.
...
MJAPI void mju_negPose(mjtNum posres[3], mjtNum quatres[4], const mjtNum pos[3],
                       const mjtNum quat[4]);
...
// Cholesky decomposition: mat = L*L'; return rank, decomposition performed
// in-place into mat.
MJAPI int mju_cholFactor(mjtNum *mat, int n, mjtNum mindiag);

// Solve (mat*mat') * res = vec, where mat is a Cholesky factor.
MJAPI void mju_cholSolve(mjtNum *res, const mjtNum *mat, const mjtNum *vec,
                         int n);
...
//  The first (ntotal-ndense) x nband store the band part, left of diagonal,
//  inclusive. The second ndense x ntotal store the band part as entire dense
//  rows. Add diagadd+diagmul*mat_ii to diagonal before factorization.
...
MJAPI void mju_band2Dense(mjtNum *res, const mjtNum *mat, int ntotal, int nband,
                          int ndense, mjtByte flg_sym);

// Convert dense matrix to banded matrix.
MJAPI void mju_dense2Band(mjtNum *res, const mjtNum *mat, int ntotal, int nband,
                          int ndense);

// Multiply band-diagonal matrix with nvec vectors, include upper triangle if
// flg_sym>0.
MJAPI void mju_bandMulMatVec(mjtNum *res, const mjtNum *mat, const mjtNum *vec,
                             int ntotal, int nband, int ndense, int nvec,
                             mjtByte flg_sym);

// Address of diagonal element i in band-dense matrix representation.
MJAPI int mju_bandDiag(int i, int ntotal, int nband, int ndense);

// Eigenvalue decomposition of symmetric 3x3 matrix, mat = eigvec * diag(eigval)
// * eigvec'.
MJAPI int mju_eig3(mjtNum eigval[3], mjtNum eigvec[9], mjtNum quat[4],
                   const mjtNum mat[9]);

// minimize 0.5*x'*H*x + x'*g  s.t. lower <= x <= upper, return rank or -1 if
// failed
//   inputs:
//     n           - problem dimension
//     H           - SPD matrix                n*n
...
//     R must have allocatd size n*(n+7), but only nfree*nfree values are used
//     in output index (if given) must have allocated size n, but only nfree
//     values are used in output only the lower triangles of H and R and are
//     read from and written to, respectively the convenience function
//     mju_boxQPmalloc allocates the required data structures
MJAPI int mju_boxQP(mjtNum *res, mjtNum *R, int *index, const mjtNum *H,
                    const mjtNum *g, int n, const mjtNum *lower,
                    const mjtNum *upper);
...
MJAPI void mju_boxQPmalloc(mjtNum **res, mjtNum **R, int **index, mjtNum **H,
                           mjtNum **g, int n, mjtNum **lower, mjtNum **upper);
...
MJAPI mjtNum mju_springDamper(mjtNum pos0, mjtNum vel0, mjtNum Kp, mjtNum Kv,
                              mjtNum dt);
...
//      C: (nsensordata x nu)
MJAPI void mjd_transitionFD(const mjModel *m, mjData *d, mjtNum eps,
                            mjtByte flg_centered, mjtNum *A, mjtNum *B,
                            mjtNum *C, mjtNum *D);

// Finite differenced Jacobians of (force, sensors) = mj_inverse(state,
// acceleration)
//   All outputs are optional. Output dimensions (transposed w.r.t Control
//   Theory convention):
//     DfDq: (nv x nv)
//     DfDv: (nv x nv)
//     DfDa: (nv x nv)
...
//     flg_actuation specifies whether to subtract qfrc_actuator from
//     qfrc_inverse
MJAPI void mjd_inverseFD(const mjModel *m, mjData *d, mjtNum eps,
                         mjtByte flg_actuation, mjtNum *DfDq, mjtNum *DfDv,
                         mjtNum *DfDa, mjtNum *DsDq, mjtNum *DsDv, mjtNum *DsDa,
                         mjtNum *DmDq);

// Derivatives of mju_subQuat.
MJAPI void mjd_subQuat(const mjtNum qa[4], const mjtNum qb[4], mjtNum Da[9],
                       mjtNum Db[9]);

// Derivatives of mju_quatIntegrate.
MJAPI void mjd_quatIntegrate(const mjtNum vel[3], mjtNum scale, mjtNum Dquat[9],
                             mjtNum Dvel[9], mjtNum Dscale[3]);
...
// If a non-identical mjpPlugin with the same name is already registered, an
// mju_error is raised. Two mjpPlugins are considered identical if all member
// function pointers and numbers are equal, and the name and attribute strings
// are all identical, however the char pointers to the strings need not be the
// same.
...
// Look up a plugin by name. If slot is not NULL, also write its registered slot
// number into it.
MJAPI const mjpPlugin *mjp_getPlugin(const char *name, int *slot);

// Look up a plugin by the registered slot number that was returned by
// mjp_registerPlugin.
...
// Globally register a resource provider in a thread-safe manner. The provider
// must have a prefix that is not a sub-prefix or super-prefix of any current
// registered providers.  This function returns a slot number > 0 on success.
...
// Return the resource provider with the prefix that matches against the
// resource name. If no match, return NULL.
MJAPI const mjpResourceProvider *
mjp_getResourceProvider(const char *resource_name);

// Look up a resource provider by slot number returned by
// mjp_registerResourceProvider. If invalid slot number, return NULL.
...
//-----------------------------------------
//
// Most MuJoCo users can ignore these functions, the following comments are
// aimed primarily at MuJoCo developers.
//
// When built and run under address sanitizer (asan), mj_markStack and
// mj_freeStack are instrumented to detect leakage of mjData stack frames. When
// the compiler inlines several callees that call into mark/free into the same
// function, this instrumentation requires that the compiler retains separate
// mark/free calls for each original callee. The memory-clobbered asm blocks act
// as a barrier to prevent mark/free calls from being combined under
// optimization.
