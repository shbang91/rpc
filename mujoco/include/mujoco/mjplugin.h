...
// callback for closing a resource (responsible for freeing any allocated
// memory)
...
typedef void (*mjfGetResourceDir)(mjResource *resource, const char **dir,
                                  int *ndir);
...
struct mjpPlugin_ {
  ...
  int capabilityflags; // plugin capabilities: bitfield of
                       // mjtPluginCapabilityBit
  ...
  // dimension of the specified sensor's output (required only for sensor
  // plugins)
  int (*nsensordata)(const mjModel *m, int instance, int sensor_id);

  // called when a new mjData is being created (required), returns 0 on success
  // or -1 on failure
  ...
  void (*reset)(const mjModel *m, double *plugin_state, void *plugin_data,
                int instance);
  ...
  // dimension of the actuator state for the plugin (excluding state from
  // actuator's dyntype)
  ...
  void (*sdf_gradient)(mjtNum gradient[3], const mjtNum point[3],
  ...
  void (*sdf_attribute)(mjtNum attribute[], const char *name[],
                        const char *value[]);
...
#define mjPLUGIN_LIB_INIT                                                      \
  ...
  mjEXTERNC int __stdcall mjDLLMAIN(void *hinst, unsigned long reason,         \
                                    void *reserved) {                          \
    if (reason == 1) {                                                         \
      _mjplugin_dllmain();                                                     \
    }                                                                          \
...
typedef void (*mjfPluginLibraryLoadCallback)(const char *filename, int first,
                                             int count);
