...
void DracoStateEstimator::UpdateModelWithGroundTruth(
    DracoSensorData *_sensor_data) {
  ...
  robot_->UpdateRobotModel(
      ...
      _sensor_data->joint_positions_, _sensor_data->joint_velocities_, true);
...
void DracoStateEstimator::UpdateModel(DracoSensorData *_sensor_data) {
  ...
  robot_->UpdateRobotModel(
      base_com_position_, Eigen::Quaternion<double>(base_joint_orientation_),
      base_com_lin_vel_, _sensor_data->imu_frame_velocities_.head(3),
      base_joint_position_, Eigen::Quaternion<double>(base_joint_orientation_),
      base_joint_lin_vel_, _sensor_data->imu_frame_velocities_.head(3),
      _sensor_data->joint_positions_, _sensor_data->joint_velocities_, true);
...
void DracoStateEstimator::EstimatePosition(DracoSensorData *_sensor_data) {
  ...
  Eigen::Vector3d anchor_frame_pos =
      robot_->GetLinkIso(sp_->stance_foot_).translation();
  Eigen::Vector3d anchor_frame_vel =
      robot_->GetLinkVel(sp_->stance_foot_).tail(3);

  if (sp_->stance_foot_ != sp_->prev_stance_foot_) {
    Eigen::Vector3d anchor_frame_pos_diff =
        anchor_frame_pos -
        robot_->GetLinkIso(sp_->prev_stance_foot_).translation();
    global_leg_odometry_ += anchor_frame_pos_diff;
  }

  // estimate position
  base_joint_position_ = global_leg_odometry_ - anchor_frame_pos;
  base_com_position_ = base_joint_position_ +
                       base_joint_orientation_ * robot_->GetBaseLocalComPos();
