...
SelectedJointTask::SelectedJointTask(RobotSystem *_robot, 
        const std::vector<std::string> &_joint_container): Task(_robot, _joint_container.size()){
...
void SelectedJointTask::UpdateOscCommand(){
    for (int i = 0; i < dim_; ++i) {
       ...
       vel_[i] = robot_->joint_velocities_[robot_->GetQdotIdx(joint_container_[i])];
...
//Link Position Task
LinkPosTask::LinkPosTask(RobotSystem *_robot, const std::string &_target_link) : Task(_robot, 3){
...
LinkOriTask::LinkOriTask(RobotSystem *_robot, const std::string &_target_link) : Task(_robot, 3){
    target_link_ = _target_link;
}
...
void LinkOriTask::UpdateOscCommand(){
    ...
    Eigen::Quaternion<double> des_quat(des_pos_[0], des_pos_[1], des_pos_[2], des_pos_[3]);
...
void LinkOriTask::UpdateTaskJacobian(){
    jacobian_ = robot_->GetLinkJacobian(target_link_).block(0, 0, dim_, robot_->n_qdot_);
}
...
void ComTask::UpdateTaskJacobian(){
