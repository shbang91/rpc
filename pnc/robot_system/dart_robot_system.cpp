...
DartRobotSystem::DartRobotSystem(const std::string &_urdf_path, const bool &_b_fixed_base, const bool &_b_print_info)
...
void DartRobotSystem::ConfigRobot(){
    if(b_fixed_base_){
        skeleton_->getRootBodyNode()->changeParentJointType<dart::dynamics::WeldJoint>();
        n_floating_base_ = 0;
    }
    ...
    if(b_print_info_){
        ...
        int count(0);
        for(std::map<std::string, dart::dynamics::JointPtr>::iterator it = joint_ptr_map_.begin();
        ...
        count = 0;
        for (std::map<std::string, dart::dynamics::BodyNodePtr>::iterator it = body_node_ptr_map_.begin(); 
...
std::map<std::string, double> DartRobotSystem::
...
Eigen::VectorXd DartRobotSystem::
...
void DartRobotSystem::UpdateRobotModel(const Eigen::Vector3d &_base_com_pos,
    ...
    if(!b_fixed_base_){
        ...
        dynamic_cast<dart::dynamics::FreeJoint *>(skeleton_->getRootJoint())
            ->setSpatialMotion(&floating_base_joint_iso, dart::dynamics::Frame::World(),
        ...
        fixed_base_joint_iso.linear() = _base_joint_quat.normalized().toRotationMatrix();
        fixed_base_joint_iso.translation() = _base_joint_pos;

        skeleton_->getRootJoint()->setTransformFromParentBodyNode(fixed_base_joint_iso);
...
Eigen::VectorXd DartRobotSystem::GetQ(){
...
Eigen::VectorXd DartRobotSystem::GetInertiaMatrix(){
...
Eigen::Isometry3d DartRobotSystem::GetLinkIso(const std::string &_link_name){
    Eigen::Isometry3d body_node_iso = body_node_ptr_map_[_link_name]->getTransform(
            dart::dynamics::Frame::World(), dart::dynamics::Frame::World()); 
    body_node_iso.translation() = body_node_ptr_map_[_link_name]->getCOM(dart::dynamics::Frame::World());
    
    return body_node_iso;
}

Eigen::Matrix<double, 6, 1> DartRobotSystem::GetLinkVel(const std::string &_link_name){
    return body_node_ptr_map_[_link_name]->getCOMSpatialVelocity(dart::dynamics::Frame::World(), dart::dynamics::Frame::World());
} 

Eigen::Matrix<double, 6, Eigen::Dynamic> DartRobotSystem::GetLinkJacobian(const std::string &_link_name){
    return skeleton_->getJacobian(body_node_ptr_map_[_link_name], body_node_ptr_map_[_link_name]->getLocalCOM(),
                                    dart::dynamics::Frame::World());

} 
Eigen::Matrix<double, 6, 1> DartRobotSystem::GetLinkJacobianDotQdot(const std::string &_link_name){
    return skeleton_->getJacobianClassicDeriv(body_node_ptr_map_[_link_name],
                                            body_node_ptr_map_[_link_name]->getLocalCOM(),
                                            dart::dynamics::Frame::World()) * GetQdot();
} 
...
Eigen::Matrix<double, 3, Eigen::Dynamic> DartRobotSystem::GetComLinJacobianDot(){
